export const adjectives = [
  'aalartig',
  'aalförmig',
  'aalglatt',
  'aasfressend',
  'aasig',
  'abaissiert',
  'abakteriell',
  'abartig',
  'abatisch',
  'abaxial',
  'abbaubar',
  'abbauwürdig',
  'abbildbar',
  'abbruchreif',
  'ABC-Waffen-frei',
  'abchasisch',
  'abderitisch',
  'abdikativ',
  'abdingbar',
  'abdominal',
  'abdominell',
  'abendfüllend',
  'abendlich',
  'abenteuerdurstig',
  'abenteuerhungrig',
  'abenteuerlich',
  'abenteuersüchtig',
  'abergläubisch',
  'aberhundert',
  'aberrant',
  'abertausend',
  'aberwitzig',
  'abessinisch',
  'abflusslos',
  'abfällig',
  'abgabenfrei',
  'abgebrüht',
  'abgefahren',
  'abgefeimt',
  'abgefuckt',
  'abgehalftert',
  'abgekartet',
  'abgeklärt',
  'abgekämpft',
  'abgelegen',
  'abgemacht',
  'abgeneigt',
  'abgerissen',
  'abgeschieden',
  'abgeschlagen',
  'abgeschlossen',
  'abgeschmackt',
  'abgängig',
  'abgöttisch',
  'abhold',
  'abhängig',
  'abiotisch',
  'abjekt',
  'abkömmlich',
  'ablandig',
  'ablativisch',
  'abnorm',
  'abnormal',
  'abominabel',
  'aboral',
  'abortiv',
  'abrasiv',
  'abrupt',
  'abscheulich',
  'abschnittsweise',
  'abschreckend',
  'abschätzig',
  'abschüssig',
  'abseitsverdächtig',
  'absent',
  'absichtlich',
  'absolut',
  'absolutistisch',
  'abstinent',
  'abstrakt',
  'abstrus',
  'absurd',
  'abträglich',
  'abtrünnig',
  'abtörnend',
  'abundant',
  'abwechslungsreich',
  'abwegig',
  'abwertend',
  'abwesend',
  'abzählbar',
  'abänderbar',
  'abänderlich',
  'achaten',
  'achromatisch',
  'achtarmig',
  'achtbar',
  'achtbändig',
  'achte',
  'achteckig',
  'achtenswert',
  'achterlich',
  'achtfach',
  'achtgeschossig',
  'achtjährig',
  'achtsam',
  'achtseitig',
  'achtstellig',
  'achtstündig',
  'achtwöchig',
  'achtzehnte',
  'achtzigste',
  'acidophil',
  'adaptiv',
  'adaxial',
  'additiv',
  'adhäsiv',
  'adiabatisch',
  'adipös',
  'adjazent',
  'adlerartig',
  'adlig',
  'administrativ',
  'adrett',
  'adsorptiv',
  'adstringierend',
  'adverbial',
  'adverbiell',
  'adygeisch',
  'adäquat',
  'aerob',
  'aerodynamisch',
  'afebril',
  'affabel',
  'affektiert',
  'affektiv',
  'affengeil',
  'affentittengeil',
  'affig',
  'affin',
  'affirmativ',
  'affrös',
  'afghanisch',
  'afrikaans',
  'afrikanisch',
  'afroamerikanisch',
  'afroasiatisch',
  'afrodeutsch',
  'agamisch',
  'agglutinierend',
  'aggressiv',
  'agil',
  'agitatorisch',
  'agnostisch',
  'agnostizistisch',
  'agogisch',
  'agonal',
  'agrarisch',
  'ahistorisch',
  'ahnungslos',
  'aidskrank',
  'aidsverseucht',
  'akatalektisch',
  'akephal',
  'akkadisch',
  'akkordisch',
  'akkumulativ',
  'akkurat',
  'akribisch',
  'aktiv',
  'aktiviert',
  'aktuell',
  'akustisch',
  'akut',
  'akzentfrei',
  'akzeptabel',
  'akzidentell',
  'al dente',
  'alabasterfarben',
  'alabastern',
  'alarmierend',
  'alaunhaltig',
  'albanisch',
  'albern',
  'albtraumhaft',
  'aleatorisch',
  'alemannisch',
  'aleppinisch',
  'alert',
  'algebraisch',
  'algerisch',
  'alimentär',
  'aliphatisch',
  'aliquant',
  'aliquot',
  'alkalisch',
  'alkoholarm',
  'alkoholfrei',
  'alkoholhaltig',
  'alkoholisch',
  'alkoholisiert',
  'alkoholselig',
  'alkoholsüchtig',
  'allabendlich',
  'allbekannt',
  'allegorisch',
  'allein',
  'alleine',
  'alleinig',
  'alleinstehend',
  'allelopathisch',
  'allenfallsig',
  'allerliebst',
  'allfällig',
  'allgegenwärtig',
  'allgemein',
  'allgemeingültig',
  'alliiert',
  'alljährlich',
  'allmonatlich',
  'allmorgendlich',
  'allmächtig',
  'allmählich',
  'allnächtlich',
  'allochthon',
  'allophonisch',
  'allstündlich',
  'alltäglich',
  'allumfassend',
  'allwissend',
  'allwöchentlich',
  'alphabetisch',
  'alphanumerisch',
  'alptraumhaft',
  'alt altbacken',
  'altbewährt',
  'altdeutsch',
  'Altdorfer',
  'altdänisch',
  'alteingesessen',
  'altenglisch',
  'alternativ',
  'alternativlos',
  'alternierend',
  'altfranzösisch',
  'altfriesisch',
  'altfränkisch',
  'altgedient',
  'altgriechisch',
  'althochdeutsch',
  'altjiddisch',
  'altklug',
  'altkrank',
  'altmodisch',
  'altnordisch',
  'altorientalisch',
  'altpreußisch',
  'altruistisch',
  'altsorbisch',
  'altsächsisch',
  'alttestamentlich',
  'altäthiopisch',
  'aluminiumhaltig',
  'alveolar',
  'amaranten',
  'ambient',
  'ambig',
  'ambitioniert',
  'ambitiös',
  'ambivalent',
  'ambulant',
  'amerikanisch',
  'amethysten',
  'amethystfarben',
  'amharisch',
  'amisch',
  'ammoniakalisch',
  'amniotisch',
  'amoralisch',
  'amorph',
  'amourös',
  'ampelografisch',
  'ampelographisch',
  'amphibolisch',
  'amphoter',
  'amtlich',
  'amtsmüde',
  'amyloid',
  'amüsant',
  'anachronistisch',
  'anaerob',
  'anakreontisch',
  'anal',
  'analgetisch',
  'analog',
  'analphabetisch',
  'analytisch',
  'anamorph',
  'anankastisch',
  'anapästisch',
  'anatomisch',
  'anbetenswert',
  'andersartig',
  'anderweitig',
  'andorranisch',
  'androgyn',
  'androphil',
  'androzentrisch',
  'anfechtbar',
  'anfänglich',
  'angeblich',
  'angeboren',
  'angeflanscht',
  'angelegentlich',
  'angelsächsisch',
  'angemessen',
  'angenehm',
  'angesagt',
  'angeschickert',
  'angesehen',
  'angetan',
  'angewiesen',
  'angewinkelt',
  'angloamerikanisch',
  'anglofranzösisch',
  'anglophon',
  'angolanisch',
  'angsteinflößend',
  'anhängig',
  'anhänglich',
  'anikonisch',
  'animalisch',
  'anlasslos',
  'anmaßend',
  'anmutig',
  'annalistisch',
  'annehmbar',
  'anodisch',
  'anomal',
  'anomisch',
  'anonym',
  'anorektisch',
  'anorganisch',
  'anotherm',
  'anoxisch',
  'anregend',
  'anregungsarm',
  'anregungsreich',
  'anschaulich',
  'anschließend',
  'ansehnlich',
  'anspruchsvoll',
  'anstellig',
  'anstrengend',
  'anständig',
  'anstößig',
  'antagonistisch',
  'antarktisch',
  'anthrazit',
  'anthrazitfarben',
  'anthropisch',
  'anthropogen',
  'anthropologisch',
  'anthropomorph',
  'anthropozentrisch',
  'antiamerikanisch',
  'antiautoritär',
  'antibakteriell',
  'antibiotikaresistent',
  'antibiotisch',
  'antibolschewistisch',
  'antichretisch',
  'antichristlich',
  'antidemokratisch',
  'antidepressiv',
  'antideutsch',
  'antieuropäisch',
  'antifaschistisch',
  'antiferromagnetisch',
  'antiguanisch',
  'antiinflammatorisch',
  'antik',
  'antiklerikal',
  'antikolonial',
  'antikommutativ',
  'antimilitaristisch',
  'antimonhaltig',
  'antinomisch',
  'antipyretisch',
  'antiquiert',
  'antirussisch',
  'antisymmetrisch',
  'antizipativ',
  'antizipiert',
  'antonymisch',
  'antriebslos',
  'anwesend',
  'anzüglich',
  'anämisch',
  'apart',
  'apathisch',
  'aper',
  'aperiodisch',
  'apfelförmig',
  'apfelgrün',
  'apikal',
  'apnoisch',
  'apodiktisch',
  'apokalyptisch',
  'apokarp',
  'apokryph',
  'apollinisch',
  'apologetisch',
  'apophatisch',
  'apoplektisch',
  'aporetisch',
  'aposteriorisch',
  'apotheotisch',
  'apotropäisch',
  'appendikuliert',
  'appetitlich',
  'appetitlos',
  'approximativ',
  'apricot',
  'aprikosenfarben',
  'apriorisch',
  'aquamarin',
  'aquamarinblau',
  'aquamarinfarben',
  'arabisch',
  'aranesisch',
  'arational',
  'arawakisch',
  'arbeitsfähig',
  'arbeitslos',
  'arbeitsmarktpolitisch',
  'arbeitsmäßig',
  'arbeitsunfähig',
  'arbiträr',
  'archaisch',
  'archetypisch',
  'architektonisch',
  'archäologisch',
  'areligiös',
  'aretologisch',
  'arg',
  'argentinisch',
  'arglos',
  'argumentativ',
  'argwöhnisch',
  'arianisch',
  'arid',
  'aristokratisch',
  'arithmetisch',
  'arkadisch',
  'arkan',
  'arktisch',
  'arm',
  'armdick',
  'armenisch',
  'armlang',
  'armselig',
  'aromatisch',
  'arriviert',
  'arrogant',
  'arschig',
  'arschkalt',
  'arschlos',
  'arsenhaltig',
  'artenreich',
  'arteriell',
  'artgerecht',
  'arthrotisch',
  'artifiziell',
  'artig',
  'artikulatorisch',
  'artistisch',
  'asbestfrei',
  'aschfarben',
  'aschkenasisch',
  'aseismisch',
  'aserbaidschanisch',
  'asexuell',
  'asiatisch',
  'askanisch',
  'asketisch',
  'askriptiv',
  'asozial',
  'assertiv',
  'assertorisch',
  'assimiliert',
  'assoziativ',
  'assyrisch',
  'asthenisch',
  'astral',
  'astrein',
  'astronomisch',
  'asturisch',
  'asymmetrisch',
  'asymptotisch',
  'asyndetisch',
  'atavistisch',
  'atembar',
  'atheistisch',
  'athermisch',
  'atlassen',
  'atomar',
  'atombetrieben',
  'atomistisch',
  'atoxisch',
  'atrophisch',
  'attraktiv',
  'attributiv',
  'atypisch',
  'aubergine',
  'auberginefarben',
  'auditiv',
  'auffallend',
  'auffällig',
  'aufgeladen',
  'aufgelöst',
  'aufgeregt',
  'aufgeräumt',
  'aufgeweckt',
  'auflandig',
  'aufmerksam',
  'aufmüpfig',
  'aufopferungsvoll',
  'aufrecht',
  'aufreibend',
  'aufrichtig',
  'aufrührerisch',
  'aufschlussreich',
  'aufsässig',
  'auftragsgemäß',
  'aufwendig',
  'aufwändig',
  'aufzählbar',
  'augenblicklich',
  'augenfällig',
  'augenscheinlich',
  'auktorial',
  'auratisch',
  'ausdruckslos',
  'ausdrucksstark',
  'ausdrucksvoll',
  'ausdrücklich',
  'ausführlich',
  'ausgeglichen',
  'ausgekocht',
  'ausgelassen',
  'ausgemergelt',
  'ausgeschildert',
  'ausgeschlossen',
  'ausgesprochen',
  'ausgestattet',
  'ausgestorben',
  'ausgewachsen',
  'ausgezeichnet',
  'ausgiebig',
  'ausheimisch',
  'auskultatorisch',
  'auslaufsicher',
  'ausländerfeindlich',
  'ausländerfrei',
  'ausländisch',
  'ausnahmslos',
  'ausreichend',
  'aussagekräftig',
  'ausschlaggebend',
  'ausschließlich',
  'ausschweifend',
  'aussichtslos',
  'ausstehend',
  'austrainiert',
  'austral',
  'australisch',
  'austrofaschistisch',
  'austronesisch',
  'ausweglos',
  'auswertbar',
  'auswählbar',
  'ausziehbar',
  'autark',
  'authentisch',
  'autistisch',
  'autobiografisch',
  'autobiographisch',
  'autochthon',
  'autodidaktisch',
  'autofrei',
  'autogen',
  'automatisch',
  'autonom',
  'autopoietisch',
  'autoritativ',
  'autoritär',
  'autosemantisch',
  'außenpolitisch',
  'außerehelich',
  'außereuropäisch',
  'außergerichtlich',
  'außergewöhnlich',
  'außerirdisch',
  'außerordentlich',
  'avantgardistisch',
  'averbal',
  'aversiv',
  'avestisch',
  'aviär',
  'awestisch',
  'axial',
  'azentrisch',
  'azephal',
  'azimutal',
  'aztekisch',
  'azurblau',
  'azurn',
  'babylonisch',
  'bacchantisch',
  'baden-württembergisch',
  'bärbeißig',
  'bärenstark',
  'bäurisch',
  'baff',
  'bahamaisch',
  'bahnbrechend',
  'bahrainisch',
  'bakteriell',
  'bakteriologisch',
  'bakteriostatisch',
  'bakterizid',
  'baldmöglichst',
  'ballsicher',
  'baltisch',
  'banal',
  'bananig',
  'bang',
  'bangladeschisch',
  'bankrott',
  'bannig',
  'bar',
  'barbusig',
  'bargeldlos',
  'barhäuptig',
  'barmherzig',
  'barock',
  'barrierefrei',
  'barsch',
  'basal',
  'basalten',
  'basalthaltig',
  'basiklin',
  'basisch',
  'baskisch',
  'basten',
  'bastfarben',
  'batisten',
  'bauchig',
  'baumlos',
  'baumreich',
  'baumwollen',
  'bauschig',
  'bayrisch',
  'bayerisch',
  'beabsichtigt',
  'beachtlich',
  'beamtet',
  'bebartet',
  'beblümt',
  'bebrillt',
  'bebuscht',
  'bebust',
  'bedauerlich',
  'bedeckt',
  'bedenkenlos',
  'bedenklich',
  'bedeutend',
  'bedeutsam',
  'bedeutungslos',
  'bedeutungsschwanger',
  'bedeutungsschwer',
  'bedeutungsvoll',
  'bedienstet',
  'bedingungslos',
  'bedonnert',
  'bedröppelt',
  'beeinflussbar',
  'beerentragend',
  'befangen',
  'befindlich',
  'beflissen',
  'befrackt',
  'befranst',
  'befremdend',
  'befremdet',
  'befremdlich',
  'befriedigend',
  'befristet',
  'befugt',
  'begabt',
  'begehbar',
  'begeißelt',
  'begeisterungsfähig',
  'begierig',
  'begrannt',
  'begründet',
  'begütert',
  'behaart',
  'behämmert',
  'behände',
  'behänd',
  'behaglich',
  'behandschuht',
  'beharrlich',
  'behavioristisch',
  'beheizbar',
  'beheizt',
  'behelmt',
  'behemdet',
  'beherzt',
  'behost',
  'behuft',
  'behutsam',
  'beidbeinig',
  'beiderseitig',
  'beidhändig',
  'beidseitig',
  'beige',
  'beigefarben',
  'beiläufig',
  'beinamputiert',
  'beinfarben',
  'beinhart',
  'beispiellos',
  'beißend',
  'bejahrt',
  'bekannt',
  'beklommen',
  'bekloppt',
  'beknackt',
  'bekömmlich',
  'bekrallt',
  'belämmert',
  'belanglos',
  'belangvoll',
  'belarussisch',
  'belastend',
  'belebt',
  'belegt',
  'belehrend',
  'belesen',
  'beleumdet',
  'beleumundet',
  'belgisch',
  'beliebig',
  'beliebt',
  'belizisch',
  'bellizistisch',
  'bemähnt',
  'bemannt',
  'bemerkenswert',
  'bemitleidenswert',
  'bemittelt',
  'bemoost',
  'bemützt',
  'benachbart',
  'beneidenswert',
  'bengalisch',
  'beninisch',
  'benthisch',
  'benutzerdefiniert',
  'benutzt',
  'bepelzt',
  'bequem',
  'beratungsresistent',
  'berechenbar',
  'berechtigt',
  'beredt',
  'bereift',
  'bereit',
  'bereitwillig',
  'bergig',
  'berindet',
  'beringt',
  'berlinerisch',
  'berlinisch',
  'bernisch',
  'bernsteinen',
  'bernsteinfarben',
  'berstig',
  'berüchtigt',
  'berühmt',
  'berüscht',
  'beruflich',
  'berufstätig',
  'berufsunfähig',
  'beschäftigt',
  'beschaffen',
  'beschaulich',
  'bescheiden',
  'bescheuert',
  'beschickert',
  'beschilft',
  'beschissen',
  'beschlussfähig',
  'beschürzt',
  'beschwerlich',
  'besenrein',
  'besoffen besondere',
  'besonnen',
  'besorgniserregend',
  'besorgt',
  'bespielbar',
  'beständig',
  'bestechlich',
  'bestiefelt',
  'bestimmt',
  'bestirnt',
  'bestrumpft',
  'bestusst',
  'betagt',
  'beteiligt',
  'betont',
  'beträchtlich',
  'betreffend',
  'betresst',
  'betreten',
  'betrieben',
  'betriebsbereit',
  'betroffen',
  'betrübt',
  'betrunken',
  'bettfertig',
  'bettlägerig',
  'betucht',
  'betulich',
  'beugbar',
  'bevölkerungsreich',
  'bewährt',
  'bewaffnet',
  'bewaldet',
  'bewandert',
  'beweglich',
  'bewegt',
  'bewehrt',
  'bewölkt',
  'bewohnbar',
  'bewundernswert',
  'bewundernswürdig',
  'bewunderungswürdig',
  'bewusst',
  'bewusstlos',
  'bezahlbar',
  'bezastert',
  'bezaubernd',
  'beziehungslos',
  'bezopft',
  'bezugnehmend',
  'bhutanisch',
  'bibliophil',
  'bibliophob',
  'biblisch',
  'bieder',
  'biereifrig',
  'bierernst',
  'bierselig',
  'bigott',
  'bijektiv',
  'bilabial',
  'bilanziell',
  'bilateral',
  'bildlich',
  'bildungsfern',
  'bildungspolitisch',
  'bildungssprachlich',
  'bilingual',
  'billiardste',
  'billig',
  'billionste',
  'bimaxillär',
  'binär',
  'binaural',
  'biografisch',
  'biographisch',
  'biologisch',
  'biologistisch',
  'biometrisch',
  'birken',
  'bisexuell',
  'bisherig',
  'bissfest',
  'bissig',
  'bistabil',
  'bitchig',
  'bitter',
  'bitterböse',
  'bitterkalt',
  'bittersüß',
  'bituminös',
  'bizarr',
  'bläulich',
  'blakig',
  'blamabel',
  'bland',
  'blank',
  'blasiert',
  'blasig',
  'blass',
  'blau',
  'blauäugig',
  'blaublütig',
  'blaugrau',
  'blaustrümpfig',
  'blechern',
  'bleich',
  'bleichsüchtig',
  'bleiern',
  'bleifarben',
  'bleigrau',
  'bleihaltig',
  'bleischwer',
  'bleu',
  'blind',
  'blitzartig',
  'blitzblank',
  'blitzeblank',
  'blitzgescheit',
  'blitzschnell',
  'blöd',
  'blöde',
  'blond',
  'bloß',
  'blümerant',
  'blütenweiß',
  'blumig',
  'blutbeschmiert',
  'blutgetränkt',
  'blutig',
  'blutjung',
  'blutleer',
  'blutrot',
  'blutrünstig',
  'bodenständig',
  'böhmisch',
  'börsennotiert',
  'bösartig',
  'böse',
  'bös',
  'bolivianisch',
  'bombig',
  'bonapartistisch',
  'bonfortionös',
  'bordeauxfarben',
  'boreal',
  'borniert',
  'borstig',
  'boshaft',
  'bosnisch',
  'bosnisch-herzegowinisch',
  'botanisch',
  'botrytisiert',
  'botsuanisch',
  'boustrophedon',
  'brach',
  'brachial',
  'brackig',
  'bräsig',
  'bräunlich',
  'brandaktuell',
  'brandenburgisch',
  'brasilianisch',
  'braun',
  'braungebrannt',
  'braungebrannt',
  'braungelb',
  'brav',
  'bravourös',
  'breiig',
  'breit',
  'breitschultrig',
  'breitschulterig',
  'bremisch',
  'brennbar',
  'brennheiß',
  'brenzlig',
  'brillant',
  'brillanten',
  'brisant',
  'britisch',
  'bröselig',
  'brokaten',
  'bromhaltig',
  'bronchial',
  'bronzefarben',
  'bronzen',
  'broschiert',
  'brüderlich',
  'brünett',
  'brünstig',
  'brüsk',
  'brummig',
  'bruneiisch',
  'brutal',
  'buchen',
  'buchhalterisch',
  'buchstabengetreu',
  'buchstäblich',
  'bucklig',
  'buddhistisch',
  'bündig',
  'bürgerlich',
  'bulgarisch',
  'bullös',
  'bundesdeutsch',
  'bundespolitisch',
  'bundesweit',
  'bundrein',
  'bunt',
  'buntfarben',
  'buntfarbig',
  'burgähnlich',
  'burgenländisch',
  'burkinisch',
  'burschenschaftlich',
  'burschikos',
  'burundisch',
  'busig',
  'bustrophedon',
  'butterweich',
  'byzantinisch',
  'CDU-geführt',
  'champagnerfarben',
  'champagnerfarbig',
  'chancenlos',
  'chaotisch',
  'chaplinesk',
  'charakteristisch',
  'charakterlos',
  'charismatisch',
  'charmant',
  'chauvinistisch',
  'chemisch',
  'chemometrisch',
  'chevaleresk',
  'chiastisch',
  'chic',
  'chilenisch',
  'chiliastisch',
  'chillig',
  'chilotisch',
  'chimärisch',
  'chinesisch',
  'chionophil',
  'chirurgisch',
  'cholerisch',
  'chondritisch',
  'christlich',
  'christologisch',
  'chronisch',
  'chronologisch',
  'cineastisch',
  'cisalpin',
  'citynah',
  'claviform',
  'clean',
  'clever',
  'cognacfarben',
  'cool',
  'costa-ricanisch',
  'cremig',
  'curricular',
  'dadaistisch',
  'dämlich',
  'dämonisch',
  'dänisch',
  'daktyloskopisch',
  'damalig',
  'damasten',
  'damisch',
  'dankbar',
  'darstellbar',
  'darwinistisch',
  'dasig',
  'datenschutzrechtlich',
  'dauerhaft',
  'dauerkrank',
  'debil',
  'debitorisch',
  'deckungsgleich',
  'dediziert',
  'deduktiv',
  'defätistisch',
  'defekt',
  'defensiv',
  'definitiv',
  'defizient',
  'defizitär',
  'degagiert',
  'degenerativ',
  'degoutant',
  'degressiv',
  'dehydriert',
  'deiktisch',
  'dekadent',
  'deklarativ',
  'deklaratorisch',
  'deklinabel',
  'deklinierbar',
  'dekremental',
  'delatorisch',
  'delektabel',
  'deletär',
  'delikat',
  'demagogisch',
  'demanten',
  'dement',
  'dementsprechend',
  'demgemäß',
  'demilitarisiert',
  'demiurgisch',
  'demografisch',
  'demographisch',
  'demokratisch',
  'demonstrativ',
  'demoskopisch',
  'demotisch',
  'demotiviert',
  'demütig',
  'denglisch',
  'denkbar',
  'denkfaul',
  'denotativ',
  'dental',
  'denunziatorisch',
  'deplatziert',
  'deplorabel',
  'deppert',
  'deprekativ',
  'depressiv',
  'depretiativ',
  'derangiert',
  'derb',
  'derivativ',
  'derogatorisch',
  'desaströs',
  'desiderat',
  'designiert',
  'desillusioniert',
  'deskriptiv',
  'desmal',
  'desolat',
  'despektierlich',
  'desperat',
  'despotisch',
  'destruktiv',
  'detailliert',
  'deterministisch',
  'deutlich',
  'deutsch',
  'deutschlandweit',
  'deutschsprachig',
  'deverbativ',
  'deviant',
  'devot',
  'dextral',
  'dezent',
  'dezentral',
  'dezidiert',
  'dezisionistisch',
  'dezisiv',
  'diabetisch',
  'diabolisch',
  'diachron',
  'diachronisch',
  'diagnostisch',
  'diagonal',
  'diakritisch',
  'dialektal',
  'dialektisch',
  'dialogarm',
  'diamanten',
  'diametral',
  'diaphan',
  'diaphasisch',
  'diastratisch',
  'diatopisch',
  'dichotom',
  'dichotomisch',
  'dicht',
  'dichterisch',
  'dick',
  'dickbäuchig',
  'dickflüssig',
  'dickleibig',
  'dicklich',
  'didaktisch',
  'diegetisch',
  'dienlich',
  'dienstfähig',
  'dienstunfähig',
  'dienstverpflichtet',
  'diesbezüglich',
  'diesig',
  'diesjährig',
  'differenzierbar',
  'diffizil',
  'diffus',
  'diffusibel',
  'digestiv',
  'digital',
  'dikasterial',
  'diktatorisch',
  'dilatorisch',
  'dilemmatisch',
  'dilettantisch',
  'dimensional',
  'dingfest',
  'dionysisch',
  'diploid',
  'diplomatisch',
  'diptotisch',
  'direkt',
  'direktional',
  'direktiv',
  'dirigistisch',
  'disjunkt',
  'diskontinuierlich',
  'diskret',
  'diskretionär',
  'diskriminierend',
  'diskursiv',
  'diskutabel',
  'disloziert',
  'disparat',
  'dispers',
  'disponibel',
  'dispositiv',
  'disputabel',
  'disruptiv',
  'dissident',
  'dissidentisch',
  'dissipativ',
  'dissonant',
  'dissozial',
  'dissoziiert',
  'distal',
  'distich',
  'distinguiert',
  'distinkt',
  'distinktiv',
  'divergent',
  'divers',
  'divinatorisch',
  'divisibel',
  'dogmatisch',
  'dolent',
  'doll',
  'dolorös',
  'dolos',
  'domänenspezifisch',
  'dominant',
  'dominikanisch',
  'dominicanisch',
  'doof',
  'doppelherzig',
  'doppelläufig',
  'doppelsinnig',
  'doppelt',
  'doppelwandig',
  'dorisch',
  'dorsal',
  'dortig',
  'dottergelb',
  'drahten',
  'drahtig',
  'drahtlos',
  'drakonisch',
  'drall',
  'dramatisch',
  'dramaturgisch',
  'drastisch',
  'dreckig',
  'drehbar',
  'dreiarmig',
  'dreibeinig',
  'dreiblättrig',
  'dreidimensional',
  'dreieckig',
  'dreifach',
  'dreihundertste',
  'dreijährig',
  'dreimonatig',
  'dreimotorig',
  'dreisilbig',
  'dreisprachig',
  'dreißigfach',
  'dreißigjährig',
  'dreißigste',
  'dreist',
  'dreistellig',
  'dreistündig',
  'dreiteilig',
  'dreitürig',
  'dreiundzwanzigste',
  'dreiwöchig',
  'dreizehnte',
  'dringend',
  'dritte',
  'drittelzahlig',
  'dröge',
  'drogensüchtig',
  'drollig',
  'drüsig',
  'dschibutisch',
  'dschihadistisch',
  'dual',
  'dubios',
  'dünkelhaft',
  'dünn',
  'dünnhäutig',
  'dürftig',
  'dürr',
  'düster',
  'dufte',
  'duktil',
  'duldsam',
  'dumm',
  'dummdreist',
  'dumpf',
  'dunkel',
  'dunkelblau',
  'dunkelfarben',
  'dunkelgrau',
  'dunkelhaarig',
  'dunkelhäutig',
  'dunkelrot',
  'dunstig',
  'duodenal',
  'durchgängig',
  'durchgebraten',
  'durchgeistigt',
  'durchgeknallt',
  'durchgreifend',
  'durchschlagend',
  'durchschnittlich',
  'durchsichtig',
  'durchtrieben',
  'durchwachsen',
  'durstig',
  'dusslig',
  'dutzendfach',
  'dyadisch',
  'dynamisch',
  'dynastisch',
  'dysfunktional',
  'dystopisch',
  'eben',
  'ebenbürtig',
  'ebenerdig',
  'ebengleich',
  'ebenholzfarben',
  'echt',
  'eckig',
  'ecuadorianisch',
  'edaphisch',
  'edel',
  'effektiv',
  'effizient',
  'egalitär',
  'egoistisch',
  'egozentrisch',
  'egressiv',
  'ehelich',
  'ehemalig',
  'ehern',
  'ehrbar',
  'ehrenamtlich',
  'ehrenhaft',
  'ehrenkäsig',
  'ehrenrührig',
  'ehrenwert',
  'ehrenwörtlich',
  'ehrgeizig',
  'ehrlich',
  'ehrlos',
  'ehrpusselig',
  'ehrwürdig',
  'eiben',
  'eichen',
  'eidetisch',
  'eierlegend',
  'eierschalenfarben',
  'eifersüchtig',
  'eifrig',
  'eigen',
  'eigenartig',
  'eigenbrötlerisch',
  'eigenhändig',
  'eigensinnig',
  'eigenständig',
  'eigensüchtig',
  'eigenwillig',
  'eilig',
  'einäugig',
  'einarmig',
  'eindeutig',
  'eindimensional',
  'eindringlich',
  'eindrucksvoll',
  'eineindeutig',
  'einfach',
  'einfältig',
  'einfallsreich',
  'einflussreich',
  'eingefleischt',
  'einhändig',
  'einheimisch',
  'einheitlich',
  'einhellig',
  'einhunderterste',
  'einhundertste',
  'einig',
  'einmalig',
  'einmotorig',
  'einprägsam',
  'einsam',
  'einschaltbereit',
  'einschlägig',
  'einseitig',
  'einsetzbar',
  'einsichtig',
  'einsilbig',
  'einsprachig',
  'einstellig',
  'einstig',
  'einstimmig',
  'einstündig',
  'eintönig',
  'einträglich',
  'einunddreißigste',
  'einundzwanzigste',
  'einvernehmlich',
  'einverstanden',
  'einwandfrei',
  'einwertig',
  'einwöchig',
  'einzellig',
  'einzeln',
  'einzig',
  'einzigartig',
  'eisenbewehrt',
  'eisenfarben',
  'eisenhaltig',
  'eisern',
  'eisfrei',
  'eisgekühlt',
  'eisig',
  'eiskalt',
  'eitel',
  'eiweißhaltig',
  'ekelerregend',
  'ekelhaft',
  'ekelig',
  'eklatant',
  'eklektisch',
  'eklig',
  'ekstatisch',
  'elaboriert',
  'elastisch',
  'elegant',
  'elegisch',
  'elektrisch',
  'elektrogen',
  'elektromagnetisch',
  'elektronisch',
  'elementar',
  'elend',
  'elenktisch',
  'eleusinisch',
  'elfeckig',
  'elfenbeinfarben',
  'elffach',
  'elfstündig',
  'elfte',
  'elitär',
  'elliptisch',
  'eloquent',
  'elterlich',
  'elternlos',
  'emblematisch',
  'emergent',
  'emeritiert',
  'eminent',
  'emisch',
  'emotional',
  'emotionslos',
  'emotiv',
  'empathisch',
  'empathogen',
  'empfänglich',
  'empfehlenswert',
  'empfindlich',
  'empfindsam',
  'emphatisch',
  'empirisch',
  'empört',
  'emsig',
  'enaktiv',
  'endemisch',
  'endgeil',
  'endgültig',
  'endlich',
  'endlos',
  'endogen',
  'endogenetisch',
  'endokrin',
  'endoplasmatisch',
  'endotherm',
  'endothym',
  'endozentrisch',
  'energetisch',
  'energiepolitisch',
  'energisch',
  'eng',
  'engagiert',
  'englisch',
  'engstirnig',
  'enkaptisch',
  'enklitisch',
  'enneaeterisch',
  'enorm',
  'enossal',
  'enostal',
  'enragiert',
  'entaktogen',
  'entartet',
  'entbehrungsreich',
  'enterohämorrhagisch',
  'entfernt',
  'entfremdet',
  'entgegenkommend',
  'enthaltsam',
  'enthusiastisch',
  'entnervt',
  'entrüstet',
  'entscheidend',
  'entschieden',
  'entschlossen',
  'entsetzlich',
  'entsprechend',
  'entsteint',
  'enttäuscht',
  'entwaffnend',
  'entzückt',
  'ephebisch',
  'ephemer',
  'epifaszial',
  'epigenetisch',
  'epigonal',
  'epigonisch',
  'epigrammatisch',
  'epikureisch',
  'episch',
  'epistemisch',
  'epistemologisch',
  'epithelial',
  'epithetisch',
  'epochal',
  'eponymisch',
  'erbärmlich',
  'erbarmungslos',
  'erblich',
  'erbötig',
  'erdähnlich',
  'erdbeerfarben',
  'erdfarben',
  'erdgeschichtlich',
  'erdhaltig',
  'erdnah',
  'erethisch',
  'erfahren',
  'erfahrungsmäßig',
  'erfolgreich',
  'erfolgswirksam',
  'erforderlich',
  'erfreulich',
  'erfrischend',
  'erfüllbar',
  'ergeben',
  'ergebnislos',
  'ergebnisoffen',
  'ergonomisch',
  'erhaben',
  'erhältlich',
  'erhalten',
  'erheblich',
  'erholsam',
  'erinnerlich',
  'eristisch',
  'eritreisch',
  'erkennbar',
  'erklecklich',
  'erlaucht',
  'erledigt',
  'erleichtert',
  'erlen',
  'erlesen',
  'erneuerbar',
  'erneut',
  'ernst',
  'ernsthaft',
  'erotisch',
  'erotomanisch',
  'erpicht',
  'erratisch',
  'erschöpft',
  'erschreckend',
  'erschwinglich',
  'erste',
  'erstrangig',
  'erträglich',
  'eruptiv',
  'erwachsen',
  'erwartungsfroh',
  'erwerbsmäßig',
  'erwünscht',
  'erythematös',
  'erzdumm',
  'erzen',
  'erzgebirgisch',
  'erzhaltig',
  'erzieherisch',
  'eschatologisch',
  'eschen',
  'eskapistisch',
  'espen',
  'essayistisch',
  'essbar',
  'essentiell',
  'essenziell',
  'estnisch',
  'etabliert',
  'etatmäßig',
  'ethisch',
  'ethnisch',
  'ethologisch',
  'etisch',
  'etruskisch',
  'etwaig',
  'etymologisch',
  'euklidisch',
  'euphemistisch',
  'euphonisch',
  'euphorisch',
  'euploid',
  'eurasisch',
  'europäisch',
  'europaweit',
  'europazentriert',
  'euroskeptisch',
  'euryhalin',
  'euryhygr',
  'euryök',
  'eurytherm',
  'euthyreot',
  'EU-weit',
  'evaluativ',
  'evangelisch',
  'evasiv',
  'eventuell',
  'evident',
  'evolutionär',
  'ewig',
  'ewiggestrig',
  'exakt',
  'exaltiert',
  'exegetisch',
  'exemplarisch',
  'exergon',
  'existentiell',
  'existenziell',
  'exklusiv',
  'exogam',
  'exogen',
  'exogenetisch',
  'exorbitant',
  'exotherm',
  'exotisch',
  'expert',
  'explizit',
  'explorativ',
  'exponentiell',
  'expressiv',
  'exquisit',
  'extensiv',
  'extern',
  'exterritorial',
  'extinkt',
  'extradiegetisch',
  'extramundan',
  'extrasolar',
  'extra-temporal',
  'extratemporal',
  'extraterrestrisch',
  'extravagant',
  'extravertiert',
  'extrem',
  'extrinsisch',
  'extrovertiert',
  'exzellent',
  'exzentrisch',
  'exzessiv',
  'fabelhaft',
  'fabisch',
  'fabrikneu',
  'facettenartig',
  'facettenreich',
  'fachgerecht',
  'fachlich',
  'fachmännisch',
  'fachsprachlich',
  'fad',
  'fadenbreit',
  'fadendünn',
  'fadenförmig',
  'fadenlos',
  'fadenscheinig',
  'fächerartig',
  'fähig',
  'fällig',
  'fälschlich',
  'fälschungssicher',
  'färöisch',
  'fahl',
  'fahnenflüchtig',
  'fahrend',
  'fahrig',
  'fahrlässig',
  'fair',
  'faktisch',
  'fakultativ',
  'fallibel',
  'fallsüchtig',
  'falsch',
  'falsifiziert',
  'familiär',
  'famos',
  'fanatisch',
  'fangfrisch',
  'fantastisch',
  'farbecht',
  'farbenblind',
  'farbenfreudig',
  'farbenfroh',
  'farbig',
  'farblos',
  'faschistisch',
  'faschistoid',
  'faselnackend',
  'fasennackend',
  'fasslich',
  'fassungslos',
  'fastidiös',
  'fatal',
  'faul',
  'faulig',
  'faustisch',
  'fazial',
  'federleicht',
  'fehlbar',
  'fehlerhaft',
  'fehlerlos',
  'feierlich',
  'feig',
  'fein',
  'feindlich',
  'feindselig',
  'feinfühlend',
  'feinfühlig',
  'feingliedrig',
  'feinkörnig',
  'feinporig',
  'feist',
  'fekund',
  'felsenfest',
  'feminin',
  'fennoskandisch',
  'fensterlos',
  'fermentativ',
  'fern',
  'fernmündlich',
  'fertig',
  'fertil',
  'fesch',
  'fesselnd',
  'fest',
  'festlich',
  'fett',
  'fettarm',
  'fetthaltig',
  'fettig',
  'fettleibig',
  'fettlöslich',
  'fettsüchtig',
  'fetzig',
  'feucht',
  'feudal',
  'feudalistisch',
  'feuerfarben',
  'feuerrot',
  'feurig',
  'fichten',
  'fickerig',
  'fidel',
  'fidschianisch',
  'fiduziarisch',
  'fieberhaft',
  'fiebrig',
  'fies',
  'figürlich',
  'figurativ',
  'figurbetont',
  'figurbewusst',
  'fiktional',
  'fiktiv',
  'filamentartig',
  'filigran',
  'filmgeschichtlich',
  'final',
  'finanziell',
  'finanzpolitisch',
  'findig',
  'finnisch',
  'finnländisch',
  'finster',
  'fipsig',
  'firn',
  'fischfressend',
  'fistelig',
  'fit',
  'fix',
  'flach',
  'flachbrüstig',
  'flachsfarben',
  'flächendeckend',
  'flächengroß',
  'flächenmäßig',
  'flächig',
  'flächsen',
  'flächsern',
  'flamboyant',
  'flanellen',
  'flapsig',
  'flaschengrün',
  'flatterhaft',
  'flau',
  'flauschig',
  'flegelhaft',
  'fleischfarben',
  'fleischig',
  'fleißig',
  'flektierend',
  'flexibel',
  'fliederfarben',
  'flink',
  'flippig',
  'floral',
  'florentinisch',
  'florid',
  'floristisch',
  'flott',
  'flüchtig',
  'flügellahm',
  'flügge',
  'flüssig',
  'fluffig',
  'fluvial',
  'fluvioglazial',
  'föderal',
  'förderlich',
  'förmlich',
  'fokal',
  'folgend',
  'folgerichtig',
  'folgewidrig',
  'folgsam',
  'forensisch',
  'formal',
  'formell',
  'formidabel',
  'formlos',
  'forsch',
  'forstwirtschaftlich',
  'fortwährend',
  'fossil',
  'fotogen',
  'foudroyant',
  'fränkisch',
  'fragil',
  'fraglich',
  'fragmentarisch',
  'fragwürdig',
  'fraisefarben',
  'fraktionslos',
  'frank',
  'französisch',
  'frappant',
  'frappierend',
  'frech',
  'frei',
  'freiberuflich',
  'freibleibend',
  'freihändig',
  'freiheitlich',
  'freiwillig',
  'freizügig',
  'fremd',
  'fremdartig',
  'fremdländisch',
  'fremdsprachig',
  'fremdsprachlich',
  'frenetisch',
  'frequent',
  'fresssüchtig',
  'freudianisch',
  'freudlos',
  'freundlich',
  'freundschaftlich',
  'frevelhaft',
  'freventlich',
  'friderizianisch',
  'friedfertig',
  'friedlich',
  'friesisch',
  'friesländisch',
  'frigide',
  'frikativ',
  'frisch',
  'fristgerecht',
  'fristlos',
  'frivol',
  'fröhlich',
  'froh',
  'frohgemut',
  'fromm',
  'frontal',
  'frostig',
  'fruchtbar',
  'fruchtig',
  'früh',
  'frühblühend',
  'frühestmöglich',
  'frühlingshaft',
  'frühneuhochdeutsch',
  'frugal',
  'fuchsteufelswild',
  'füllig',
  'fünfeckig',
  'fünffach',
  'fünfhundertste',
  'fünfprozentig',
  'fünfstellig',
  'fünfstündig',
  'fünfte',
  'fünftürig',
  'fünfwöchig',
  'fünfzehnte',
  'fünfzigste',
  'fürchterlich',
  'fürstlich',
  'fulminant',
  'fummelig',
  'fundamental',
  'fungibel',
  'funktional',
  'funktionsfähig',
  'funzelig',
  'funzlig',
  'furchtbar',
  'furchterregend',
  'furchtlos',
  'furchtsam',
  'furios',
  'furztrocken',
  'fußläufig',
  'gabunisch',
  'gängig',
  'gänzlich',
  'galant',
  'gallertartig',
  'gallertig',
  'gallig',
  'gallisch',
  'galloromanisch',
  'galvanisch',
  'gambisch',
  'gangbar',
  'ganz',
  'ganzheitlich',
  'ganzjährig',
  'ganztägig',
  'gar',
  'garstig',
  'gasförmig',
  'gashaltig',
  'gasig',
  'gastfreundlich',
  'gastral',
  'gastrisch',
  'gastrointestinal',
  'gealtert',
  'geartet',
  'gebalkt',
  'gebenedeit',
  'gebildet',
  'gebirgig',
  'geboren',
  'geborgen',
  'gebräuchlich',
  'gebraucht',
  'gebrechlich',
  'gebrochen',
  'gebuchtet',
  'gebuckelt',
  'gebührenfrei',
  'gebührenpflichtig',
  'gebürtig',
  'gechillt',
  'gedankenlos',
  'gedankenverloren',
  'gedeckt',
  'gediegen',
  'gedrungen',
  'geduldig',
  'geeignet',
  'gefährlich',
  'gefällig',
  'gefahrlos',
  'gefahrvoll',
  'gefasst',
  'gefiedert',
  'gefinkelt',
  'gefizt',
  'geflissentlich',
  'geflügelt',
  'gefriergetrocknet',
  'gefühlsbetont',
  'gegengleich',
  'gegenseitig',
  'gegenwärtig',
  'gegiebelt',
  'gehässig',
  'gehäusetragend',
  'gehalten',
  'geheim',
  'geheimnisvoll',
  'geheist',
  'gehenkelt',
  'gehl',
  'gehoben',
  'gehörig',
  'gehörlos',
  'gehorsam',
  'geil',
  'geisterhaft',
  'geistesabwesend',
  'geisteskrank',
  'geistig',
  'geistlich',
  'geistreich',
  'geizig',
  'geköpert',
  'gekröpft',
  'gelassen',
  'gelaunt',
  'gelb',
  'gelbbraun',
  'gelblich',
  'gelbsüchtig',
  'geldhungrig',
  'geldpolitisch',
  'geleckt',
  'gelegen',
  'gelegentlich',
  'gelehrt',
  'gelenkig',
  'gelinde',
  'gell',
  'geltungssüchtig',
  'gemächlich',
  'gemäß',
  'gemein',
  'gemeinfasslich',
  'gemeinfrei',
  'gemeinnützig',
  'gemeinsam',
  'gemeinslawisch',
  'gemeinverständlich',
  'gemütlich',
  'genau',
  'geneigt',
  'generativ',
  'generell',
  'generisch',
  'generös',
  'genetisch',
  'genial',
  'genotypisch',
  'genügend',
  'genügsam',
  'genuesisch',
  'genuin',
  'genusssüchtig',
  'geografisch',
  'geographisch',
  'geologisch',
  'geometrisch',
  'geopolitisch',
  'georgisch',
  'geowissenschaftlich',
  'geozentrisch',
  'geperlt',
  'gepunktet',
  'gerade',
  'gerädert',
  'geräuschlos',
  'geraum',
  'gerecht',
  'gerichtlich',
  'geriffelt',
  'gering',
  'geringfügig',
  'gerissen',
  'germanisch',
  'gerollt',
  'geruchlos',
  'gerundet',
  'gesamt',
  'gesamtdeutsch',
  'geschäftig',
  'geschätzt',
  'geschafft',
  'gescheckt',
  'gescheit',
  'geschert',
  'geschichtslos',
  'geschichtsträchtig',
  'geschickt',
  'geschlechtslos',
  'geschlechtsneutral',
  'geschlechtsreif',
  'geschlossen',
  'geschmacklich',
  'geschmacklos',
  'geschmacksneutral',
  'geschmackvoll',
  'geschmäcklerisch',
  'geschmeidig',
  'geschoren',
  'geschwätzig',
  'geschwind',
  'gesellig',
  'gesellschaftlich',
  'gesetzeswidrig',
  'gesetzlich',
  'gesetzwidrig',
  'gespannt',
  'gesprenkelt',
  'geständig',
  'gestanden',
  'gestiefelt',
  'gestielt',
  'gestirnt',
  'gestorben',
  'gestreift',
  'gestreng',
  'gestrig',
  'gesucht',
  'gesund',
  'gesundheitspolitisch',
  'gesundheitsschädlich',
  'getreu',
  'gewalmt',
  'gewaltig',
  'gewaltlos',
  'gewaltsam',
  'gewalttätig',
  'gewandt',
  'gewerblich',
  'gewerbsmäßig',
  'gewichtig',
  'gewillkürt',
  'gewillt',
  'gewinnsüchtig',
  'gewiss',
  'gewissenhaft',
  'gewöhnlich',
  'gewöhnungsbedürftig',
  'gewohnheitsmäßig',
  'gewohnt',
  'gezielt',
  'ghanaisch',
  'giftgrün',
  'gifthaltig',
  'giftig',
  'gigantisch',
  'gigantomanisch',
  'gingival',
  'glänzend',
  'gläsern',
  'glamourös',
  'glanzhell',
  'glasig',
  'glatt',
  'glatzköpfig',
  'glaubhaft',
  'glaubwürdig',
  'gleich',
  'gleichaltrig',
  'gleichartig',
  'gleichgültig',
  'gleichmäßig',
  'gleichmütig',
  'gleichnamig',
  'gleichschenklig',
  'gleichwertig',
  'gleichzeitig',
  'glimpflich',
  'glitschig',
  'global',
  'glockenförmig',
  'glockenhell',
  'glorreich',
  'glottal',
  'glücklich',
  'glückselig',
  'gluteal',
  'glutenfrei',
  'gnadenlos',
  'gnädig',
  'gönnerhaft',
  'göttlich',
  'golden',
  'goldfarben',
  'goldgelb',
  'goldhaltig',
  'goldig',
  'gotisch',
  'gotteslästerlich',
  'gottgefällig',
  'gottlos',
  'gottverdammt',
  'graduell',
  'grässlich',
  'grätig',
  'gräulich',
  'grafisch',
  'grafisch',
  'graphisch',
  'grafitfarben',
  'graphitfarben',
  'grafithaltig',
  'graphithaltig',
  'grammatikalisch',
  'grammatisch',
  'grandios',
  'graniten',
  'grantig',
  'graphematisch',
  'graphitfarben',
  'grasgrün',
  'grau',
  'graublau',
  'grauenhaft',
  'graufarben',
  'grausam',
  'gravierend',
  'gravitätisch',
  'grazil',
  'graziös',
  'gregorianisch',
  'greifbar',
  'greis',
  'grell',
  'grellfarben',
  'grenadisch',
  'grenzdebil',
  'grenzenlos',
  'griechisch',
  'grifffest',
  'griffig',
  'grimmig',
  'grindig',
  'griveliert',
  'grob',
  'grobgemahlen',
  'grobkörnig',
  'grobporig',
  'grönländisch',
  'größenwahnsinnig',
  'größtmöglich',
  'groß',
  'großartig',
  'großbritannisch',
  'großflächig',
  'großherzig',
  'großkotzig',
  'großmütig',
  'großzügig',
  'grotesk',
  'grottenschlecht',
  'grottig',
  'grün',
  'gründlich',
  'grünlich',
  'grundfalsch',
  'grundgütig',
  'grundlegend',
  'grundlos',
  'grundsätzlich',
  'grundständig',
  'gruselig',
  'guatemaltekisch',
  'gülden',
  'gültig',
  'günstig',
  'gütig',
  'guinea-bissauisch',
  'guineisch',
  'gusseisern',
  'gustatorisch',
  'gut',
  'guttural',
  'guyanisch',
  'gymnastisch',
  'gynäkologisch',
  'gynozentrisch',
  'haarähnlich',
  'haarartig',
  'haargenau',
  'haarig',
  'haarsträubend',
  'habitabel',
  'habituell',
  'hackedicht',
  'hadernhaltig',
  'hämisch',
  'hämorrhagisch',
  'händisch',
  'hären',
  'hässlich',
  'häufig',
  'häuslich',
  'hager',
  'haitianisch',
  'halb',
  'halbblind',
  'halb fertig',
  'halbfertig',
  'halbjährig',
  'halbjährlich',
  'halbnackt',
  'halboffiziell',
  'halbseiden',
  'halbstaatlich',
  'halbstark',
  'halbstündig',
  'halbstündlich',
  'halbtägig',
  'halbtäglich',
  'halbverklungen',
  'halbwollen',
  'hallisch',
  'halluzinogen',
  'haloniert',
  'halophil',
  'haltbar',
  'haltig',
  'hammerhart',
  'handfest',
  'handgeführt',
  'handgreiflich',
  'handlich',
  'handlungsunfähig',
  'handsam',
  'handschriftlich',
  'handwerklich',
  'handyfrei',
  'hanebüchen',
  'hanfen',
  'hanseatisch',
  'hansisch',
  'hapaxanth',
  'haploid',
  'happig',
  'haptisch',
  'harmlos',
  'harsch',
  'hart',
  'hartgesotten',
  'hartnäckig',
  'hasserfüllt',
  'hastig',
  'hauchdünn',
  'hauptamtlich',
  'hauptberuflich',
  'hauptsächlich',
  'hausgemacht',
  'haushaltspolitisch',
  'hauteng',
  'hautfarben',
  'hautfreundlich',
  'hautschonend',
  'hautverträglich',
  'havariert',
  'hawaiisch',
  'hebräisch',
  'hedonisch',
  'hedonistisch',
  'heftig',
  'hegelianisch',
  'hegemonial',
  'hehr',
  'heidnisch',
  'heikel',
  'heil',
  'heilfroh',
  'heilig',
  'heillos',
  'heimelig',
  'heimisch',
  'heimlich',
  'heiser',
  'heiß',
  'heißgeliebt',
  'heiter',
  'hektisch',
  'heldenhaft',
  'heldisch',
  'hell',
  'hellblau',
  'hellblond',
  'hellenistisch',
  'hellfarben',
  'hellgelb',
  'hellgrün',
  'hellhäutig',
  'helllicht',
  'hellrot',
  'hellsichtig',
  'hellwach',
  'hemdsärmelig',
  'hemizygot',
  'hepatisch',
  'hepatotoxisch',
  'herablassend',
  'heranwachsend',
  'herausragend',
  'herb',
  'herbstfarben',
  'herbstlich',
  'hereditär',
  'herkömmlich',
  'hermeneutisch',
  'hermetisch',
  'heroisch',
  'herrlich',
  'herrschsüchtig',
  'hervorragend',
  'herzförmig',
  'herzlich',
  'herzlos',
  'hessisch',
  'heterogen',
  'heterosexuell',
  'heterozygot',
  'heuchlerisch',
  'heuristisch',
  'hexagonal',
  'hieb- und stichfest',
  'hieratisch',
  'hiesig',
  'hilfreich',
  'hilfsbereit',
  'himbeerfarben',
  'himmelblau',
  'hinfällig',
  'hinnehmbar',
  'hinreichend',
  'hinterfotzig',
  'hinterhältig',
  'hip',
  'hirschledern',
  'historisch',
  'hitzig',
  'HIV-negativ',
  'HIV-positiv',
  'hoch',
  'hochachtungsvoll',
  'hochaktuell',
  'hochauflösend',
  'hochbeglückt',
  'hochbetagt',
  'hochdeutsch',
  'hochdosiert',
  'hochdramatisch',
  'hochelegant',
  'hochgeachtet',
  'hochgelobt',
  'hochgradig',
  'hochhackig',
  'hochherzig',
  'hochinteressant',
  'hochkompliziert',
  'hochmodern',
  'hochmögend',
  'hochnäsig',
  'hochschwanger',
  'hochsprachlich',
  'hochtrabend',
  'hochverräterisch',
  'hochverschuldet',
  'hochwertig',
  'hochwohlgeboren',
  'hochzeitlich',
  'höchstpersönlich',
  'höflich',
  'höhnisch',
  'hölzern',
  'hörbar',
  'hoffähig',
  'hoffärtig',
  'hoffnungslos',
  'hohl',
  'hold',
  'holistisch',
  'holländisch',
  'holprig',
  'holperig',
  'holzfrei',
  'holzhaltig',
  'homerisch',
  'homiletisch',
  'homöostatisch',
  'homogen',
  'homoiohydrisch',
  'homolog',
  'homonuklear',
  'homorgan',
  'homosexuell',
  'homozygot',
  'honduranisch',
  'honigfarben',
  'honigsüß',
  'honorig',
  'horizontal',
  'horrend',
  'hübsch',
  'hühnermüde',
  'humanitär',
  'humanoid',
  'humid',
  'humil',
  'humoral',
  'humorig',
  'humoristisch',
  'humorlos',
  'humorvoll',
  'hunderterste',
  'hundertfach',
  'hundertprozentig',
  'hundertste',
  'hunderttausendste',
  'hundertundzweite',
  'hundertzweite',
  'hundsgemein',
  'hungrig',
  'hunsrückisch',
  'hurtig',
  'hybrid',
  'hydrogen',
  'hydrophil',
  'hydrophob',
  'hydroponisch',
  'hygrisch',
  'hygroskopisch',
  'hypermetropisch',
  'hypertroph',
  'hypoäolisch',
  'hypodorisch',
  'hypoionisch',
  'hypolokrisch',
  'hypolydisch',
  'hypomixolydisch',
  'hypophrenisch',
  'hypophrygisch',
  'hypothetisch',
  'hypsometrisch',
  'hysterisch',
  'ideal',
  'ideell',
  'idempotent',
  'identisch',
  'ideografisch',
  'ideographisch',
  'ideologisch',
  'idiographisch',
  'idiomatisch',
  'idiopathisch',
  'idiosynkratisch',
  'idiotensicher',
  'ignobel',
  'ikonisch',
  'ikonoklastisch',
  'illegal',
  'illiquid',
  'illokutionär',
  'illokutiv',
  'illusionär',
  'illusorisch',
  'illuster',
  'imaginär',
  'imbezil',
  'immanent',
  'immatur',
  'immediat',
  'immens',
  'immergrün',
  'immobil',
  'immun',
  'impardonnabel',
  'imperfektiv',
  'imperialistisch',
  'impertinent',
  'imperturbatisch',
  'implizit',
  'importun',
  'impotent',
  'impulsiv',
  'inadäquat',
  'inbegriffen',
  'inbrünstig',
  'inchoativ',
  'indefinibel',
  'indefinit',
  'indeklinabel',
  'indianisch',
  'indifferent',
  'indigen',
  'indigniert',
  'indigofarben',
  'indirekt',
  'indisch',
  'indiskret',
  'indiskutabel',
  'indisponiert',
  'individuell',
  'indoeuropäisch',
  'indogermanisch',
  'indolent',
  'indonesisch',
  'indossierbar',
  'induktiv',
  'industriell',
  'induzierbar',
  'ineffabel',
  'ineffektiv',
  'ineffizient',
  'inert',
  'infam',
  'infantil',
  'infaust',
  'infernalisch',
  'infinitesimal',
  'inflammatorisch',
  'inflationär',
  'informativ',
  'informell',
  'infrarot',
  'inguinal',
  'inhärent',
  'inhaltlich',
  'inhaltslos',
  'initial',
  'injektiv',
  'inkohärent',
  'inkommensurabel',
  'inkommod',
  'inkompetent',
  'inkompressibel',
  'inkongruent',
  'inkremental',
  'inkrementell',
  'innenpolitisch',
  'innig',
  'inniglich',
  'innovativ',
  'inoffiziell',
  'insektenfressend',
  'inselbegabt',
  'insistent',
  'insolent',
  'instabil',
  'inständig',
  'instantan',
  'institutionell',
  'insuffizient',
  'intakt',
  'integer',
  'integrativ',
  'integrierbar',
  'intellektuell',
  'intelligent',
  'intelligibel',
  'intendiert',
  'intensional',
  'intensiv',
  'intentional',
  'interaktiv',
  'interdenominational',
  'interdisziplinär',
  'interessant',
  'interessiert',
  'interglazial',
  'interimistisch',
  'interkontinental',
  'interkostal',
  'interkranial',
  'interkrustal',
  'interkulturell',
  'interkurrent',
  'interkurrierend',
  'interlinear',
  'interlingual',
  'intermediär',
  'intermenstrual',
  'intermenstruell',
  'intermittierend',
  'intermolekular',
  'intern',
  'internal',
  'international',
  'internetsüchtig',
  'interorbital',
  'interozeanisch',
  'interparlamentarisch',
  'interpersonal',
  'interpersonell',
  'interplanetar',
  'interplanetarisch',
  'interpoliert',
  'interpretationsfähig',
  'interpretativ',
  'interpretierbar',
  'interrogativ',
  'intersektoral',
  'intersexuell',
  'interstadial',
  'interstellar',
  'interstitiell',
  'intersubjektiv',
  'interterritorial',
  'interurban',
  'intervallisch',
  'interventionistisch',
  'interventiv',
  'intervertebral',
  'intervokalisch',
  'interzellulär',
  'interzellular',
  'intestinal',
  'intim',
  'intolerant',
  'intramolekular',
  'intransigent',
  'intransitiv',
  'intraossär',
  'intravasal',
  'intravenös',
  'intrigant',
  'intrikat',
  'intrinsisch',
  'introspektiv',
  'introvertiert',
  'intuitiv',
  'invariant',
  'invasiv',
  'invers',
  'invertiert',
  'investigativ',
  'investiv',
  'inzident',
  'iodhaltig',
  'ionisch',
  'ionotrop',
  'ipsativ',
  'ipsilateral',
  'irakisch',
  'iranisch',
  'irden',
  'irdisch',
  'irisch',
  'irisierend',
  'irländisch',
  'irokesisch',
  'ironisch',
  'irr',
  'irrational',
  'irreal',
  'irrelevant',
  'irreparabel',
  'irreversibel',
  'isenthalp',
  'isentrop',
  'isländisch',
  'islamisch',
  'islamistisch',
  'islamitisch',
  'isochor',
  'isochron',
  'isodiametrisch',
  'isokinetisch',
  'isolationistisch',
  'isometrisch',
  'isomorph',
  'isotonisch',
  'isotrop',
  'israelisch',
  'israelitisch',
  'italienisch',
  'italisch',
  'iterativ',
  'ithyphallisch',
  'ivorisch',
  'jadegrün',
  'jaden',
  'jägersprachlich',
  'jäh',
  'jährig',
  'jährlich',
  'jähzornig',
  'jämmerlich',
  'jagdbar',
  'jahrelang',
  'jahrhundertelang',
  'jahrtausendelang',
  'jahrzehntelang',
  'jakobinisch',
  'jamaikanisch',
  'jambisch',
  'janusköpfig',
  'japanisch',
  'javanisch',
  'jeansfarben',
  'jemenitisch',
  'jenisch',
  'jetzig',
  'jeweilig',
  'jiddisch',
  'jiddischistisch',
  'jodhaltig',
  'jordanisch',
  'journalistisch',
  'jovial',
  'jubelnd',
  'judaistisch',
  'judenspanisch',
  'judiziös',
  'jüdisch',
  'jugendfrei',
  'jugendlich',
  'jugendsprachlich',
  'julianisch',
  'jung',
  'jungenhaft',
  'juridisch',
  'juristisch',
  'justiziabel',
  'juvenil',
  'juwelen',
  'juxtarenal',
  'kabbalistisch',
  'kabellos',
  'kaduk',
  'kälteliebend',
  'kärntisch',
  'kärntnerisch',
  'käuflich',
  'kaffeebraun',
  'kafkaesk',
  'kahl',
  'kaiserlich',
  'kakifarben',
  'kakophonisch',
  'kalendertäglich',
  'kalkhaltig',
  'kalorienbewusst',
  'kalorisch',
  'kalt',
  'kaltschnäuzig',
  'kambodschanisch',
  'kamerascheu',
  'kameraüberwacht',
  'kamerunisch',
  'kampfeslustig',
  'kampflos',
  'kanadisch',
  'kanarisch',
  'kanevassen',
  'kanonisch',
  'kantonesisch',
  'kantoniert',
  'kanzerogen',
  'kapazitiv',
  'kapital',
  'kapriziös',
  'kaputt',
  'kapuzenförmig',
  'kapverdisch',
  'karamellfarben',
  'karelisch',
  'karg',
  'karibisch',
  'kariert',
  'karikaturartig',
  'karikaturistisch',
  'kariogen',
  'karitativ',
  'karminativ',
  'karolingisch',
  'karolinisch',
  'karthagisch',
  'kasachisch',
  'kastanienbraun',
  'kastenförmig',
  'kasuistisch',
  'katalanisch',
  'katalytisch',
  'katarisch',
  'katastrophal',
  'katatonisch',
  'kategorisch',
  'katenativ',
  'katholisch',
  'katotherm',
  'kattunen',
  'kaudal',
  'kaudinisch',
  'kaufmännisch',
  'kaufsüchtig',
  'kausal',
  'kautelarjuristisch',
  'keck',
  'kegelförmig',
  'kehlig',
  'keilförmig',
  'kelchförmig',
  'keltisch',
  'kenianisch',
  'kerngesund',
  'kernlos',
  'kerzengerade',
  'kerzengrade',
  'kess',
  'ketzerisch',
  'keusch',
  'khakifarben',
  'kiebig',
  'kiefern',
  'kieshaltig',
  'kiesig',
  'kinästhetisch',
  'kinderleicht',
  'kinderlos',
  'kindgemäß',
  'kindgerecht',
  'kindisch',
  'kindlich',
  'kinematographisch',
  'kirgisisch',
  'kiribatisch',
  'kirre',
  'kirschrot',
  'kitschig',
  'kitzelig',
  'kitzlig',
  'kläglich',
  'klamm',
  'klammheimlich',
  'klandestin',
  'klapprig',
  'klapperig',
  'klar',
  'klassisch',
  'klassizistisch',
  'klastisch',
  'klatschnass',
  'klatschsüchtig',
  'klebrig',
  'kleiehaltig',
  'klein',
  'kleinbürgerlich',
  'kleinkariert',
  'kleinlaut',
  'kleinlich',
  'klerikal',
  'klinisch',
  'klitschig',
  'klitschnass',
  'klitzeklein',
  'klobig',
  'klug',
  'knackig',
  'knallhart',
  'knapp',
  'knauserig',
  'knechtisch',
  'knietief',
  'knifflig',
  'knitz',
  'knollig',
  'knorrig',
  'knuddelig',
  'knuffig',
  'koalitionstreu',
  'koaxial',
  'koblenzisch',
  'kochsalzhaltig',
  'koedukativ',
  'königlich',
  'königsblau',
  'körnerfressend',
  'körperlich',
  'köstlich',
  'koffeinhaltig',
  'kognitiv',
  'kohärent',
  'kohlehaltig',
  'kohlensäurehaltig',
  'kohlenstoffhaltig',
  'kohlpechrabenschwarz',
  'kohlrabenschwarz',
  'kokett',
  'kollateral',
  'kollektiv',
  'kolossal',
  'kolumbianisch',
  'komatös',
  'kombinatorisch',
  'kometenhaft',
  'komfortabel',
  'komisch',
  'kommend',
  'kommentarlos',
  'kommerziell',
  'kommissarisch',
  'kommissiv',
  'kommod',
  'kommunikativ',
  'kommunistisch',
  'kommutativ',
  'komorbid',
  'komorisch',
  'kompakt',
  'komparabel',
  'kompatibel',
  'kompetent',
  'kompetitiv',
  'komplektisch',
  'komplementär',
  'komplett',
  'komplex',
  'kompliziert',
  'kompositorisch',
  'kompromissbereit',
  'kompromisslos',
  'kompulsiv',
  'komputativ',
  'konditional',
  'konfirmatorisch',
  'konfliktär',
  'konfluent',
  'konform',
  'konfrontativ',
  'konfus',
  'kongenial',
  'kongenital',
  'kongolesisch',
  'kongruent',
  'konisch',
  'konjugal',
  'konkav',
  'konkludent',
  'konkomitant',
  'konkordant',
  'konkret',
  'konkurrenzfähig',
  'konnatal',
  'konphas',
  'konsekutiv',
  'konsensual',
  'konsequent',
  'konservativ',
  'konsistent',
  'konsonant',
  'konsonantisch',
  'konspirativ',
  'konstant',
  'konsterniert',
  'konstitutionell',
  'konstitutiv',
  'konstruktiv',
  'kontagiös',
  'kontaktscheu',
  'kontaminiert',
  'kontemplativ',
  'kontemporär',
  'kontextfrei',
  'kontextsensitiv',
  'kontextuell',
  'kontingent',
  'kontinuierlich',
  'kontradiktorisch',
  'konträr',
  'kontraindiziert',
  'kontraintuitiv',
  'kontraproduktiv',
  'kontrastarm',
  'kontrastreich',
  'kontrovers',
  'konvenient',
  'konventionell',
  'konvergent',
  'konvers',
  'konvex',
  'konvivial',
  'konvulsiv',
  'konvulsivisch',
  'konzentrisch',
  'konzeptionell',
  'konzeptuell',
  'konzertiert',
  'konzessiv',
  'konziliant',
  'konzis',
  'kooperativ',
  'kopiergeschützt',
  'korallen',
  'koranisch',
  'koreanisch',
  'korkig',
  'kornblumenblau',
  'koronal',
  'korporiert',
  'korpulent',
  'korrekt',
  'korrektiv',
  'korrigierbar',
  'korrosionsbeständig',
  'korrumpierbar',
  'korrupt',
  'kortikal',
  'kosakisch',
  'koscher',
  'kosmetisch',
  'kosmisch',
  'kosmopolitisch',
  'kosovarisch',
  'kosovo-albanisch',
  'kostenfrei',
  'kostengünstig',
  'kostenlos',
  'kostenmäßig',
  'kostenpflichtig',
  'kotranslational',
  'kovalent',
  'kräftezehrend',
  'kräftig',
  'kraftlos',
  'kraftvoll',
  'krampfig',
  'kranial',
  'krank',
  'krankenhausreif',
  'krankenversichert',
  'krankhaft',
  'krass',
  'krautig',
  'kreativ',
  'krebserregend',
  'kredibel',
  'kreditorisch',
  'kreditwürdig',
  'kreidebleich',
  'kreidehaltig',
  'kreiden',
  'kreisförmig',
  'kreisfrei',
  'kretazisch',
  'kreuzbrav',
  'kreuzfidel',
  'kreuzlahm',
  'kriegerisch',
  'kriegsbeschädigt',
  'kriegsgeplagt',
  'kriegstüchtig',
  'kriegsversehrt',
  'kriminell',
  'kriminogen',
  'krisenfest',
  'krisengebeutelt',
  'krisengeschüttelt',
  'krisensicher',
  'krisselig',
  'krisslig',
  'kristallen',
  'kristallklar',
  'kritikfähig',
  'kritisch',
  'kroatisch',
  'kross',
  'krude',
  'krütsch',
  'krumm',
  'kryophil',
  'kryptisch',
  'kryptographisch',
  'kryptomer',
  'kubanisch',
  'kühl',
  'kühn',
  'kündbar',
  'künftig',
  'künstlerisch',
  'künstlich',
  'kugelförmig',
  'kugelsicher',
  'kulant',
  'kulinarisch',
  'kultisch',
  'kulturell',
  'kulturpolitisch',
  'kumpelhaft',
  'kumulativ',
  'kundig',
  'kunstgeschichtlich',
  'kunstseiden',
  'kupferhaltig',
  'kupfern',
  'kurdisch',
  'kurios',
  'kurrent',
  'kursiv',
  'kursorisch',
  'kurz',
  'kurzärmelig',
  'kurzärmlig',
  'kurzfristig',
  'kurzlebig',
  'kurzsichtig',
  'kurzweilig',
  'kussecht',
  'kuwaitisch',
  'kymrisch',
  'kyrillisch',
  'labial',
  'labil',
  'labiodental',
  'labyrinthartig',
  'labyrinthisch',
  'lächerlich',
  'länglich',
  'läppisch',
  'lässig',
  'lässlich',
  'lästig',
  'läufig',
  'laff',
  'laikal',
  'laizistisch',
  'lakonisch',
  'lakustrisch',
  'lamarckistisch',
  'laminar',
  'lammfromm',
  'landespolitisch',
  'landesweit',
  'landläufig',
  'landschaftlich',
  'landständisch',
  'landwirtschaftlich',
  'lang',
  'langärmelig',
  'langärmlig',
  'langatmig',
  'langfristig',
  'langgestreckt',
  'langjährig',
  'langlebig',
  'langsam',
  'langstängelig',
  'langstänglig',
  'langstielig',
  'langweilig',
  'langwierig',
  'laotisch',
  'lapidar',
  'larmoyant',
  'laryngeal',
  'lasch',
  'lasziv',
  'lateinisch',
  'latent',
  'lateral',
  'latinisch',
  'latreutisch',
  'lau',
  'launig',
  'launisch',
  'lausig',
  'laut',
  'lauter',
  'lautlos',
  'lautmalerisch',
  'lautstark',
  'lauwarm',
  'lavendelfarben',
  'lax',
  'lebendgebärend',
  'lebendig',
  'lebenserfahren',
  'lebenslänglich',
  'lebenslustig',
  'lebhaft',
  'leblos',
  'leck',
  'lecker',
  'leckerfritzig',
  'ledern',
  'ledig',
  'leer',
  'legal',
  'legasthen',
  'legasthenisch',
  'legendär',
  'leger',
  'legitim',
  'lehmhaltig',
  'lehrreich',
  'leichenblass',
  'leicht',
  'leichtfertig',
  'leichtsinnig',
  'leid',
  'leidenschaftlich',
  'leidlich',
  'leidsam',
  'leinen',
  'leinwand',
  'leise',
  'leistungsstark',
  'leiwand',
  'leninistisch',
  'leprös',
  'lesbar',
  'lesbisch',
  'leserlich',
  'lesothisch',
  'letal',
  'lethargisch',
  'lettisch',
  'letzt',
  'leutselig',
  'levantinisch',
  'levurozid',
  'lexikalisch',
  'lexikographisch',
  'libanesisch',
  'liberal',
  'liberianisch',
  'libertär',
  'libidinös',
  'libysch',
  'licht',
  'lichtdurchlässig',
  'lichterloh',
  'lichtgrau',
  'lichtundurchlässig',
  'lidschäftig',
  'lieb',
  'liebenswürdig',
  'liebesfähig',
  'liebeskrank',
  'liebestoll',
  'liebevoll',
  'lieblich',
  'liechtensteinisch',
  'liederlich',
  'lila',
  'limnisch',
  'limnologisch',
  'limnophil',
  'lind',
  'linden',
  'lindgrün',
  'linear',
  'linguistisch',
  'linientreu',
  'liniert',
  'liniiert',
  'link',
  'linkisch',
  'linksalternativ',
  'linksdrehend',
  'linkslastig',
  'linksrekursiv',
  'linksrheinisch',
  'linsenförmig',
  'lipophil',
  'lipophob',
  'listig',
  'listrisch',
  'litauisch',
  'literal',
  'literarisch',
  'lithologisch',
  'liturgisch',
  'livid',
  'lizenziert',
  'lobenswert',
  'locker',
  'löblich',
  'logisch',
  'logografisch',
  'logographisch',
  'lohfarben',
  'lohnenswert',
  'lokal',
  'lokrisch',
  'lokutionär',
  'lokutiv',
  'longitudinal',
  'los',
  'lotrecht',
  'loyal',
  'lucianisch',
  'ludisch',
  'lübsch',
  'lückenlos',
  'lüdisch',
  'lügnerisch',
  'lüstern',
  'lütt',
  'luftdicht',
  'luftdurchlässig',
  'luftgekühlt',
  'lukrativ',
  'lukulent',
  'lukullisch',
  'luminös',
  'lunar',
  'lupenrein',
  'lustgetrieben',
  'lustig',
  'lustlos',
  'luxemburgisch',
  'luxuriös',
  'luzid',
  'lydisch',
  'lyrisch',
  'machbar',
  'machiavellistisch',
  'machtlos',
  'machtvoll',
  'madagassisch',
  'madegassisch',
  'madig',
  'madjarisch',
  'mäandrisch',
  'mächtig',
  'mädchenhaft',
  'männermordend',
  'männlich',
  'märchenhaft',
  'märkisch',
  'mäßig',
  'mäzenatisch',
  'mafiös',
  'mafios',
  'magensaftresistent',
  'mager',
  'magersüchtig',
  'maghrebinisch',
  'magisch',
  'magmatisch',
  'magyarisch',
  'mainzisch',
  'maisgelb',
  'majorenn',
  'makaber',
  'makedonisch',
  'makellos',
  'makroskopisch',
  'malad',
  'malade',
  'malawisch',
  'malaysisch',
  'maledivisch',
  'maligne',
  'malisch',
  'maliziös',
  'maltesisch',
  'malthusisch',
  'mamertinisch',
  'mandatorisch',
  'mandibular',
  'manganhaltig',
  'mangelhaft',
  'manichäisch',
  'manieriert',
  'manierlich',
  'manipulativ',
  'manisch',
  'mannhaft',
  'mannigfaltig',
  'mannmännlich',
  'mannstoll',
  'manuell',
  'marastisch',
  'marginal',
  'marin',
  'marinblau',
  'maritim',
  'markant',
  'markenrechtlich',
  'markig',
  'marktbar',
  'marktwirtschaftlich',
  'marmorn',
  'marode',
  'marokkanisch',
  'marshallisch',
  'martialisch',
  'marxistisch',
  'maschinenlesbar',
  'maschinenmäßig',
  'maskulin',
  'masochistisch',
  'massenhaft',
  'massentauglich',
  'maßgebend',
  'maßgeblich',
  'massig',
  'massiv',
  'masturbatorisch',
  'masurisch',
  'materiell',
  'mathematisch',
  'matriarchalisch',
  'matt',
  'mattgolden',
  'mau',
  'maulfaul',
  'mauretanisch',
  'maurisch',
  'mauritisch',
  'mausetot',
  'mautfrei',
  'mautpflichtig',
  'maxillar',
  'maximal',
  'mazedonisch',
  'mechanisch',
  'mediäval',
  'medial',
  'median',
  'mediat',
  'mediengeschichtlich',
  'medienhistorisch',
  'medienpolitisch',
  'medienwirksam',
  'medikamentös',
  'medioker',
  'mediterran',
  'medizinisch',
  'megaloman',
  'megalomanisch',
  'mehrdeutig',
  'mehrfach',
  'mehrgleisig',
  'mehrjährig',
  'mehrmalig',
  'mehrmonatig',
  'mehrschichtig',
  'mehrsilbig',
  'mehrsprachig',
  'mehrspurig',
  'mehrstündig',
  'mehrtägig',
  'mehrwöchig',
  'meißenisch',
  'meißnisch',
  'meistbesucht',
  'meisterhaft',
  'meistgeklickt',
  'melancholisch',
  'melanistisch',
  'meliert',
  'meliorativ',
  'melodisch',
  'menschenleer',
  'menschlich',
  'mental',
  'meritokratisch',
  'merkantil',
  'merkantilistisch',
  'merkmalhaltig',
  'merkmallos',
  'merkwürdig',
  'meschugge',
  'mesial',
  'messbar',
  'messingen',
  'metabolisch',
  'metabotrop',
  'metadiegetisch',
  'metallhaltig',
  'metallic',
  'metallisch',
  'metaphorisch',
  'metaphysisch',
  'metastabil',
  'metatheoretisch',
  'meterdick',
  'methodologisch',
  'metonymisch',
  'metrisch',
  'metrosexuell',
  'mexikanisch',
  'mickrig',
  'mies',
  'miesepetrig',
  'mikronesisch',
  'mikroskopisch',
  'mild',
  'militärisch',
  'militant',
  'millenarisch',
  'milliardste',
  'millionenfach',
  'millionste',
  'mimetisch',
  'mimosenhaft',
  'minderjährig',
  'minderwertig',
  'minimal',
  'minimalistisch',
  'ministrabel',
  'minütlich',
  'minutenlang',
  'minutiös',
  'minuziös',
  'mirakulös',
  'miserabel',
  'misogyn',
  'missgelaunt',
  'missgünstig',
  'missionarisch',
  'misslich',
  'missmutig',
  'misstrauisch',
  'missvergnügt',
  'mitleiderregend',
  'mitleidig',
  'mitschuldig',
  'mitteilsam',
  'mittel',
  'mittelalterig',
  'mittelaltrig',
  'mittelalterlich',
  'mittelamerikanisch',
  'mittelbar',
  'mitteldeutsch',
  'mittelenglisch',
  'mitteleuropäisch',
  'mittelfränkisch',
  'mittelfranzösisch',
  'mittelfristig',
  'mittelgriechisch',
  'mittelhochdeutsch',
  'mittellateinisch',
  'mittelmäßig',
  'mittelniederdeutsch',
  'mittelniederländisch',
  'mitverantwortlich',
  'mixolydisch',
  'mnemonisch',
  'mnemotechnisch',
  'mnestisch',
  'mobil',
  'modal',
  'modebewusst',
  'moderat',
  'moderativ',
  'modern',
  'modrig',
  'modular',
  'möglich',
  'mörderisch',
  'mokant',
  'moldauisch',
  'molekular',
  'mollig',
  'momentan',
  'monadisch',
  'monadologisch',
  'monastisch',
  'monatelang',
  'monatlich',
  'mondän',
  'mondlos',
  'mondsüchtig',
  'monegassisch',
  'monetär',
  'mongolid',
  'mongolisch',
  'mongoloid',
  'monochrom',
  'monohybrid',
  'monokausal',
  'monolingual',
  'monolithisch',
  'monomolekular',
  'monomorph',
  'monophyletisch',
  'monosyllabisch',
  'monotheistisch',
  'monoton',
  'montan',
  'montenegrinisch',
  'monumental',
  'moralinsauer',
  'moralisch',
  'morastig',
  'morbid',
  'mordsmäßig',
  'morganatisch',
  'morgendlich',
  'morgig',
  'moros',
  'morphematisch',
  'morphemisch',
  'morphiumsüchtig',
  'morphologisch',
  'morphosyntaktisch',
  'morsch',
  'mosambikanisch',
  'moselfränkisch',
  'moselromanisch',
  'motivational',
  'motiviert',
  'motorisch',
  'motorisiert',
  'mottenzerfressen',
  'mucksmäuschenstill',
  'müde',
  'mühsam',
  'mündig',
  'mündlich',
  'mürbe',
  'mürrisch',
  'müßig',
  'mütterlich',
  'muffig',
  'mukös',
  'mulmig',
  'multikausal',
  'multikulturell',
  'multilateral',
  'multilingual',
  'multimedial',
  'multinational',
  'multipel',
  'multitaskingfähig',
  'mundartlich',
  'mundgerecht',
  'mundtot',
  'munitionsverseucht',
  'munizipal',
  'munter',
  'museal',
  'musikalisch',
  'musisch',
  'muskulär',
  'muskulös',
  'mutig',
  'mutlos',
  'mutmaßlich',
  'mutterseelenallein',
  'muttersprachlich',
  'mutwillig',
  'myanmarisch',
  'myop',
  'myopisch',
  'mysteriös',
  'mystisch',
  'mythologisch',
  'nachbörslich',
  'nachdrücklich',
  'nachfolgend',
  'nachgiebig',
  'nachhaltig',
  'nachlässig',
  'nachmalig',
  'nachrichtlich',
  'nachtaktiv',
  'nachteilig',
  'nachträglich',
  'nachtragend',
  'nachvollziehbar',
  'nachweisbar',
  'nachweislich',
  'nackt',
  'nächst',
  'nächtlich',
  'nämlich',
  'närrisch',
  'nagelneu',
  'nah',
  'nahrhaft',
  'naiv',
  'namenlos',
  'namentlich',
  'namibisch',
  'napoleonisch',
  'narrativ',
  'narrensicher',
  'narzisstisch',
  'nasal',
  'naschhaft',
  'naschsüchtig',
  'naseweis',
  'nass',
  'nassforsch',
  'nasskalt',
  'national',
  'nationalbewusst',
  'nationalistisch',
  'nativ',
  'natriumarm',
  'natürlich',
  'naturalistisch',
  'nauruisch',
  'nautisch',
  'neapolitanisch',
  'nebelhaft',
  'nebelverhangen',
  'nebenamtlich',
  'nebenberuflich',
  'nebensächlich',
  'neblig',
  'nebulös',
  'neckisch',
  'negativ',
  'neidisch',
  'neidlos',
  'neidvoll',
  'nennenswert',
  'neodarwinistisch',
  'neolamarckistisch',
  'neonatologisch',
  'neongrün',
  'neovitalistisch',
  'nepalesisch',
  'nephrotoxisch',
  'neritisch',
  'nervig',
  'nervös',
  'nett',
  'netzpolitisch',
  'neu',
  'neuartig',
  'neudeutsch',
  'neugeboren',
  'neugierig',
  'neuhochdeutsch',
  'neulateinisch',
  'neumodisch',
  'neuneckig',
  'neunfach',
  'neunmalklug',
  'neunstellig',
  'neunstündig',
  'neunte',
  'neunwöchig',
  'neunzehnte',
  'neunzigste',
  'neuralgisch',
  'neurasthenisch',
  'neurotisch',
  'neuseeländisch',
  'neusilbern',
  'neusumerisch',
  'neutestamentlich',
  'neutral',
  'neuwertig',
  'nicaraguanisch',
  'nichtig',
  'nichtnegativ',
  'nichtperturbativ',
  'nichtsnutzig',
  'nieder',
  'niederdeutsch',
  'niedergeschlagen',
  'niederländisch',
  'niederösterreichisch',
  'niederrheinisch',
  'niedersorbisch',
  'niederträchtig',
  'niedlich',
  'niedrig',
  'nigelnagelneu',
  'nigerianisch',
  'nigrisch',
  'nikaraguanisch',
  'nikotinhaltig',
  'nikotinsüchtig',
  'nillenkrank',
  'nival',
  'niveaulos',
  'nobel',
  'nördlich',
  'nötig',
  'nominal',
  'nomologisch',
  'nomothetisch',
  'nonchalant',
  'nonkonform',
  'nonverbal',
  'nordafrikanisch',
  'nordamerikanisch',
  'norddeutsch',
  'nordelbisch',
  'nordeuropäisch',
  'nordgermanisch',
  'nordisch',
  'nordkoreanisch',
  'nordöstlich',
  'nordrhein-westfälisch',
  'nordwestdeutsch',
  'nordwestlich',
  'normal',
  'normativ',
  'normgerecht',
  'normwidrig',
  'norwegisch',
  'nostalgisch',
  'notgeil',
  'notorisch',
  'notwendig',
  'nubisch',
  'nüchtern',
  'nützlich',
  'nuklear',
  'nukleophil',
  'nullte',
  'numerisch',
  'nutzbar',
  'nutzlos',
  'obdachlos',
  'oberaffengeil',
  'oberdeutsch',
  'oberflächlich',
  'obergärig',
  'oberirdisch',
  'oberösterreichisch',
  'obersächsisch',
  'oberschlächtig',
  'obersorbisch',
  'obig',
  'objektiv',
  'objektorientiert',
  'obligat',
  'obligatorisch',
  'obliviös',
  'obskur',
  'obsolet',
  'obstinat',
  'obstruktiv',
  'obszön',
  'öd',
  'ödipal',
  'öffenbar',
  'öffentlich',
  'ökologisch',
  'ökonomisch',
  'ölhaltig',
  'ölig',
  'önologisch',
  'örtlich',
  'österlich',
  'österreichisch',
  'österreichweit',
  'östlich',
  'ofenfrisch',
  'offen',
  'offenbar',
  'offenkundig',
  'offensichtlich',
  'offensiv',
  'offiziell',
  'offizinal',
  'offizinell',
  'offiziös',
  'ogygisch',
  'ohnmächtig',
  'ohrenbetäubend',
  'okkasionell',
  'okklusal',
  'okkult',
  'oknophil',
  'oktaedrisch',
  'oktroyiert',
  'okular',
  'okzipital',
  'okzitanisch',
  'olfaktorisch',
  'olympisch',
  'omanisch',
  'ominös',
  'omnipotent',
  'omnipräsent',
  'omniszient',
  'omnivor',
  'onomasiologisch',
  'onomatopoetisch',
  'ontogenetisch',
  'ontologisch',
  'opak',
  'opalen',
  'operational',
  'operativ',
  'ophthalmisch',
  'opiumsüchtig',
  'opportun',
  'opportunistisch',
  'optimal',
  'optimistisch',
  'optional',
  'optisch',
  'opulent',
  'oral',
  'orange',
  'orbital',
  'ordentlich',
  'ordinär',
  'ordnungsgemäß',
  'ordnungsmäßig',
  'ordnungspolitisch',
  'ordoliberal',
  'oreal',
  'organisational',
  'organisch',
  'organisiert',
  'orgiastisch',
  'originär',
  'original',
  'originell',
  'orographisch',
  'orthodox',
  'orthognath',
  'orthogonal',
  'orthographisch',
  'orthotrop',
  'ortsgebunden',
  'ortskundig',
  'ortsunkundig',
  'oskisch',
  'osmanisch',
  'osmotisch',
  'ostdeutsch',
  'ostensibel',
  'ostensiv',
  'ostentativ',
  'ostfälisch',
  'ostgermanisch',
  'ostgrönländisch',
  'ostjiddisch',
  'ostpreußisch',
  'oszillierend',
  'oval',
  'ovarial',
  'ovipar',
  'ovovivipar',
  'ozonhaltig',
  'ozonhältig',
  'paarig',
  'packend',
  'pädagogisch',
  'pädophil',
  'pagan',
  'pagatorisch',
  'pakistanisch',
  'palästinensisch',
  'palatal',
  'palato-alveolar',
  'palauisch',
  'palliativ',
  'pampig',
  'panamaisch',
  'panchronisch',
  'pandemisch',
  'panegyrisch',
  'panhellenisch',
  'panikartig',
  'pantomimisch',
  'papabel',
  'papabile',
  'papierdeutsch',
  'papieren',
  'papierlos',
  'papiern',
  'pappsatt',
  'papstfähig',
  'papua-neuguineisch',
  'paradiesisch',
  'paradigmatisch',
  'paradox',
  'paradoxal',
  'paraguayisch',
  'paralinguistisch',
  'parallel',
  'paramagnetisch',
  'parametrisch',
  'paramilitärisch',
  'paranoid',
  'paraphyletisch',
  'parasitär',
  'parasitisch',
  'parasprachlich',
  'parat',
  'paraverbal',
  'paraxial',
  'parenteral',
  'parietal',
  'pariserisch',
  'paritätisch',
  'parochial',
  'parteiintern',
  'parteiisch',
  'parteilos',
  'parteinah',
  'parteipolitisch',
  'parthisch',
  'partial',
  'partiell',
  'partikular',
  'partizipativ',
  'partizipatorisch',
  'passabel',
  'passager',
  'passend',
  'passiert',
  'passiv',
  'pastellen',
  'pastös',
  'pastoral',
  'patent',
  'patentiert',
  'paternalistisch',
  'pathetisch',
  'pathogen',
  'pathognomonisch',
  'pathologisch',
  'patriarchalisch',
  'patriotisch',
  'patrologisch',
  'patschnass',
  'patt',
  'patzig',
  'paulinisch',
  'pausbäckig',
  'pauschal',
  'pausenlos',
  'pechschwarz',
  'pedantisch',
  'peinlich',
  'pejorativ',
  'pekig',
  'pektoral',
  'pekuniär',
  'pelagisch',
  'pelzig',
  'pendent',
  'penetrant',
  'penibel',
  'penil',
  'pentelisch',
  'perfekt',
  'perfektiv',
  'perfide',
  'performant',
  'performativ',
  'pergamenisch',
  'pergamenten',
  'perimortal',
  'perinatal',
  'periodisch',
  'peripatetisch',
  'peripher',
  'periphrastisch',
  'perkussiv',
  'perkutan',
  'perlmuttern',
  'perlokutionär',
  'perlokutiv',
  'permanent',
  'permeabel',
  'permissiv',
  'permutabel',
  'perpendikular',
  'perpetuell',
  'perplex',
  'persisch',
  'persistent',
  'persönlich',
  'persuasiv',
  'peruanisch',
  'pervasiv',
  'pervers',
  'perzeptiv',
  'pessimistisch',
  'pestilenzartig',
  'pfälzisch',
  'pfeilschnell',
  'pfenniggenau',
  'pfiffig',
  'pfirsichfarben',
  'pflanzlich',
  'pflegebedürftig',
  'pfleglich',
  'pflichtbewusst',
  'pflichtvergessen',
  'pflichtversichert',
  'phänomenal',
  'phänomenologisch',
  'phänotypisch',
  'phallisch',
  'phaneromer',
  'phantastisch',
  'philippinisch',
  'philiströs',
  'philologisch',
  'philosophisch',
  'phlegmatisch',
  'phonematisch',
  'phonemisch',
  'phonetisch',
  'phonologisch',
  'phonotaktisch',
  'phosphathaltig',
  'photogen',
  'phrygisch',
  'phyletisch',
  'phylogenetisch',
  'physikalisch',
  'physiologisch',
  'physisch',
  'phytosanitär',
  'phytotoxisch',
  'pickelig',
  'piefig',
  'piekfein',
  'pieksig',
  'pietätlos',
  'pikant',
  'pikaresk',
  'pikarisch',
  'pikiert',
  'pimpelig',
  'pingelig',
  'pink',
  'pithekoid',
  'pitschepatschenass',
  'pitschnass',
  'pittoresk',
  'plakativ',
  'plan',
  'planar',
  'planmäßig',
  'plantar',
  'plastisch',
  'platinhaltig',
  'platonisch',
  'platt',
  'plattdeutsch',
  'platzmäßig',
  'plausibel',
  'plebejisch',
  'plerophor',
  'plietsch',
  'plötzlich',
  'plosiv',
  'plüschen',
  'plump',
  'pluralisch',
  'pluralistisch',
  'pluripotent',
  'plurizentrisch',
  'plutonisch',
  'pneumatisch',
  'poemisch',
  'poetisch',
  'polabisch',
  'polemisch',
  'polemogen',
  'politisch',
  'polnisch',
  'polydispers',
  'polygam',
  'polyglott',
  'polymorph',
  'polyphyletisch',
  'polysem',
  'polysynthetisch',
  'polytheistisch',
  'polytoxikoman',
  'polytrop',
  'polyvalent',
  'polyzentrisch',
  'polyzyklisch',
  'pompös',
  'pongid',
  'populär',
  'populistisch',
  'pornografisch',
  'pornographisch',
  'portabel',
  'portofrei',
  'portugiesisch',
  'porzellanen',
  'porzin',
  'positiv',
  'possessiv',
  'possierlich',
  'postalisch',
  'postalveolar',
  'postfrisch',
  'posthum',
  'postkommunistisch',
  'postlagernd',
  'postmortal',
  'postnatal',
  'postoperativ',
  'postprandial',
  'postulationsfähig',
  'postwendend',
  'potent',
  'potentiell',
  'potentiometrisch',
  'potenziell',
  'potenzsteigernd',
  'potthässlich',
  'prachtvoll',
  'prächtig',
  'prädestiniert',
  'prädikativ',
  'prädiktabel',
  'prägnant',
  'prähistorisch',
  'präjudiziell',
  'präliminär',
  'prämiert',
  'prämonetär',
  'pränatal',
  'präplanetar',
  'präpositional',
  'präpotent',
  'präprandial',
  'prärogativ',
  'präsent',
  'präsentisch',
  'präsidiabel',
  'präsidial',
  'präskriptiv',
  'präsumtiv',
  'prätentiös',
  'präterital',
  'präventiv',
  'präzis',
  'pragmatisch',
  'praktisch',
  'prall',
  'preisgekrönt',
  'preiswert',
  'prekär',
  'prestigeträchtig',
  'preußisch',
  'prickelnd',
  'priesterlich',
  'prim',
  'primär',
  'primitiv',
  'primordial',
  'prinzipiell',
  'prioritär',
  'privat',
  'proaktiv',
  'probabilistisch',
  'probat',
  'problematisch',
  'problemlos',
  'prodromal',
  'produktiv',
  'profan',
  'professionell',
  'profitabel',
  'profund',
  'prognath',
  'programmatisch',
  'progredient',
  'progressiv',
  'prollig',
  'prominent',
  'promisk',
  'promiskuitiv',
  'prompt',
  'propädeutisch',
  'proper',
  'prophylaktisch',
  'proportional',
  'proprietär',
  'prosaisch',
  'prosodisch',
  'prosopografisch',
  'prospektiv',
  'proteisch',
  'protektionistisch',
  'prototypisch',
  'provenzalisch',
  'provinziell',
  'provisorisch',
  'provokant',
  'proximal',
  'prüde',
  'prunksüchtig',
  'prunkvoll',
  'pseudonym',
  'pseudowissenschaftlich',
  'psychedelisch',
  'psychisch',
  'psychosomatisch',
  'psychosozial',
  'psychrophil',
  'ptolemäisch',
  'publizistisch',
  'pudelnass',
  'pünktlich',
  'pueril',
  'puerperal',
  'pulmologisch',
  'pulmonal',
  'pulverig',
  'pulvrig',
  'punitiv',
  'punktuell',
  'pur',
  'puristisch',
  'puritanisch',
  'purpurn',
  'purulent',
  'putativ',
  'putzig',
  'pyknisch',
  'pyrogen',
  'pyroklastisch',
  'pythonesk',
  'quabbelig',
  'quaderförmig',
  'quadratisch',
  'quadrillionste',
  'quälend',
  'qualitativ',
  'quantenphysikalisch',
  'quantifizierbar',
  'quantitativ',
  'quarzhaltig',
  'quarzhältig',
  'quasistationär',
  'quasistatisch',
  'quecksilberhaltig',
  'quecksilberhältig',
  'quicklebendig',
  'quietschfidel',
  'quinär',
  'quirlig',
  'quittengelb',
  'quittegelb',
  'quotal',
  'rabbinisch',
  'rabiat',
  'radial',
  'radikal',
  'radioaktiv',
  'radiumhaltig',
  'räß',
  'rätselhaft',
  'räudig',
  'räumdig',
  'räumlich',
  'raffgierig',
  'raffiniert',
  'rahmig',
  'rahn',
  'rallig',
  'rammdösig',
  'randlos',
  'rangig',
  'rank',
  'ranzig',
  'rar',
  'rasant',
  'rasch',
  'rasend',
  'raß',
  'rastlos',
  'ratierlich',
  'rational',
  'rationell',
  'ratsam',
  'rau',
  'raubauzig',
  'rauchig',
  'rauschgiftsüchtig',
  'reaktionär',
  'reaktiv',
  'real',
  'realistisch',
  'realitätsfremd',
  'realpolitisch',
  'rebenumsponnen',
  'rebsortenrein',
  'recht',
  'rechteckig',
  'rechthaberisch',
  'rechtlich',
  'rechtmäßig',
  'rechtschaffen',
  'rechtsdrehend',
  'rechtsfrei',
  'rechtspolitisch',
  'rechtspopulistisch',
  'rechtsrheinisch',
  'rechtswissenschaftlich',
  'rechtwinkelig',
  'rechtwinklig',
  'rechtzeitig',
  'redegewandt',
  'redselig',
  'reduktionistisch',
  'redundant',
  'reell',
  'reflexiv',
  'refraktär',
  'rege',
  'regelgemäß',
  'regelgerecht',
  'regelkonform',
  'regelmäßig',
  'regelrecht',
  'regelwidrig',
  'regennass',
  'regierungskritisch',
  'regierungsnah',
  'regimetreu',
  'regional',
  'reglos',
  'regnerisch',
  'regressiv',
  'regulär',
  'rehbraun',
  'reibungslos',
  'reich',
  'reichhaltig',
  'reichlich',
  'reif',
  'rein',
  'reinlich',
  'reinrassig',
  'reisefähig',
  'reiselustig',
  'reißerisch',
  'reißfest',
  'reizbar',
  'reizend',
  'reizlos',
  'reizvoll',
  'rekordverdächtig',
  'rektal',
  'rekursiv',
  'relativ',
  'relevant',
  'religiös',
  'religionsphilosophisch',
  'renaissancistisch',
  'renitent',
  'renommiert',
  'renovierungsbedürftig',
  'rentabel',
  'reparabel',
  'reparierbar',
  'repetitiv',
  'repräsentativ',
  'repressiv',
  'republikanisch',
  'reputabel',
  'reputierlich',
  'reserviert',
  'resigniert',
  'resistent',
  'resolut',
  'resonant',
  'resorbierbar',
  'respektabel',
  'respektiv',
  'respektlos',
  'respektvoll',
  'restituiert',
  'restitutiv',
  'restlich',
  'restlos',
  'restriktiv',
  'restringiert',
  'resultatlos',
  'retardierend',
  'retikulär',
  'retinotop',
  'retransloziert',
  'retroaktiv',
  'retrobulbär',
  'retrofuturistisch',
  'retrograd',
  'retrospektiv',
  'reuig',
  'reumütig',
  'revanchistisch',
  'reversibel',
  'revolutionär',
  'rezent',
  'rezeptfrei',
  'rezeptiv',
  'rezeptorvermittelt',
  'rezeptpflichtig',
  'rezessiv',
  'rezidivierend',
  'reziprok',
  'rhapsodisch',
  'rheinfränkisch',
  'rheinisch',
  'rheinländisch',
  'rheinland-pfälzisch',
  'rheophil',
  'rhetorisch',
  'rhodiniert',
  'rhombisch',
  'rhomboedrisch',
  'rhythmisch',
  'richterlich',
  'richtig',
  'richtungsabhängig',
  'ridikül',
  'riechbar',
  'riesengroß',
  'riesenhaft',
  'riesig',
  'rigide',
  'rigoros',
  'ringförmig',
  'ripuarisch',
  'risikoaffin',
  'risikoarm',
  'risikoavers',
  'risikolos',
  'risikoreich',
  'riskant',
  'rissfest',
  'rissig',
  'ritterbürtig',
  'ritterlich',
  'rituell',
  'robust',
  'rodelfrei',
  'römisch',
  'römisch-deutsch',
  'römisch-katholisch',
  'röntgenabsorbierend',
  'rösch',
  'rötlich',
  'rötlichbraun',
  'rötlichgelb',
  'roh',
  'rollig',
  'romanhaft',
  'romanisch',
  'romanistisch',
  'romantisch',
  'romfreundlich',
  'rosa',
  'rosarot',
  'rosé',
  'rosenrot',
  'rosig',
  'rossig',
  'rostfrei',
  'rostig',
  'rostral',
  'rot',
  'rotblau',
  'rotbraun',
  'rotgefärbt',
  'rotgelb',
  'rotglühend',
  'rotgolden',
  'rotgrün',
  'rothaarig',
  'rotwangig',
  'rotwelsch',
  'rotzfrech',
  'rotzig',
  'routinemäßig',
  'routiniert',
  'royal',
  'ruandisch',
  'rubinrot',
  '',
  'ruchbar',
  'ruchlos',
  'ruckartig',
  'rudimentär',
  'rückblickend',
  'rückgängig',
  'rückhaltlos',
  'rückläufig',
  'rücklaufend',
  'rückschrittlich',
  'rückseitig',
  'rücksichtslos',
  'rücksichtsvoll',
  'rückständig',
  'rückwärtig',
  'rückwärtsgekrümmt',
  'rückwärtsgewandt',
  'rückwirkend',
  'rüde',
  'rügenswert',
  'rührig',
  'rührselig',
  'rüpelhaft',
  'rüstern',
  'rüstig',
  'ruhelos',
  'ruhig',
  'ruhmsüchtig',
  'ruhmvoll',
  'rumänisch',
  'rund',
  'rundlich',
  'runisch',
  'runtergerockt',
  'runzelig',
  'ruppig',
  'rußig',
  'russisch',
  'russischstämmig',
  'russlanddeutsch',
  'rustikal',
  'rutschig',
  'saarländisch',
  'sachbezogen',
  'sachdienlich',
  'sachgerecht',
  'sachlich',
  'sacht',
  'sackleinen',
  'säbelbeinig',
  'sächlich',
  'sächsisch',
  'sägerau',
  'säkular',
  'sämig',
  'sämtlich',
  'säuberlich',
  'säumig',
  'säurehaltig',
  'sagenhaft',
  'sagenumwoben',
  'sagittal',
  'sahnig',
  'saisonal',
  'sakral',
  'sakrosankt',
  'salomonisch',
  'salonfähig',
  'salopp',
  'salpeterhaltig',
  'salvadorianisch',
  'salzarm',
  'salzburgerisch',
  'salzburgisch',
  'salzfrei',
  'salzhaltig',
  'salzig',
  'salzlos',
  'sambisch',
  'samoanisch',
  'samten',
  'samtig',
  'samtweich',
  'sandig',
  'sanft',
  'sanguinisch',
  'san-marinesisch',
  'saphirblau',
  'saphiren',
  'sapphisch',
  'sardinisch',
  'sardisch',
  'sardonisch',
  'sarkastisch',
  'satanisch',
  'satt',
  'satzwertig',
  'sauber',
  'saubillig',
  'saudi-arabisch',
  'saudisch',
  'sauer',
  'sauerstoffarm',
  'sauerstoffhaltig',
  'sauertöpfisch',
  'saumselig',
  'sauteuer',
  'schablonenhaft',
  'schachmatt',
  'schade',
  'schadenfroh',
  'schäbig',
  'schädlich',
  'schändlich',
  'schal',
  'schamhaft',
  'schamlos',
  'scharf',
  'scharfzüngig',
  'scharlachen',
  'scharlachrot',
  'schartig',
  'schattenhaft',
  'schattig',
  'schau',
  'schaurig',
  'scheel',
  'scheinbar',
  'scheinheilig',
  'scheißegal',
  'scheißfreundlich',
  'schematisch',
  'schemenhaft',
  'scherzhaft',
  'scheu',
  'scheußlich',
  'schiach',
  'schick',
  'schicker',
  'schiech',
  'schief',
  'schiefern',
  'schier',
  'schiitisch',
  'schilfen',
  'schilfleinen',
  'schimärisch',
  'schimmlig',
  'schismogen',
  'schizophren',
  'schläfrig',
  'schlaff',
  'schlaftrunken',
  'schlagfertig',
  'schlagzeilenträchtig',
  'schlampig',
  'schlangenförmig',
  'schlank',
  'schlau',
  'schlecht',
  'schleckig',
  'schleierhaft',
  'schleswig-holsteinisch',
  'schleunig',
  'schlicht',
  'schlimm',
  'schlohweiß',
  'schlüpfrig',
  'schlüsselfertig',
  'schlüssig',
  'schmackhaft',
  'schmächtig',
  'schmal',
  'schmallippig',
  'schmerzenvoll',
  'schmerzhaft',
  'schmerzlich',
  'schmerzvoll',
  'schmiedbar',
  'schmierig',
  'schmuck',
  'schmutzig',
  'schnäukig',
  'schneebedeckt',
  'schneeblind',
  'schneefrei',
  'schneereich',
  'schneeweiß',
  'schneidig',
  'schnell',
  'schnelllebig',
  'schnellstmöglich',
  'schnieke',
  'schnodderig',
  'schnoddrig',
  'schnöde',
  'schnurgerade',
  'schnurrbärtig',
  'schön',
  'schofel',
  'schofelig',
  'schoflig',
  'schokoladen',
  'schottisch',
  'schräg',
  'schreckensblass',
  'schreckhaft',
  'schrecklich',
  'schreibfaul',
  'schriftlich',
  'schriftlos',
  'schriftsprachlich',
  'schrittweise',
  'schroff',
  'schrullig',
  'schrundig',
  'schüchtern',
  'schütter',
  'schulautonom',
  'schuldbewusst',
  'schuldenfrei',
  'schuldig',
  'schulfrei',
  'schummrig',
  'schuppig',
  'schurwollen',
  'schussbereit',
  'schusselig',
  'schussfest',
  'schusslig',
  'schwach',
  'schwachsinnig',
  'schwäbisch',
  'schwächlich',
  'schwätzicht',
  'schwammig',
  'schwanger',
  'schwarz',
  'schwarzäugig',
  'schwarzfeldrig',
  'schwarzrotgolden',
  'Schwarzwälder',
  'schwatzhaft',
  'schwedisch',
  'schwefelhaltig',
  'schweigsam',
  'schweinchenrosa',
  'schweinekalt',
  'schweinern',
  'schweineteuer',
  'schweizerisch',
  'schwer',
  'schwerelos',
  'schwerkriegsbeschädigt',
  'schwerlötig',
  'schwerpunktmäßig',
  'schwerreich',
  'schwerwiegend',
  'schwielig',
  'schwierig',
  'schwindelfrei',
  'schwindelig',
  'schwindsüchtig',
  'schwül',
  'schwul',
  'schwungvoll',
  'sechsarmig',
  'sechseckig',
  'sechsfach',
  'sechsstellig',
  'sechsstündig',
  'sechste',
  'sechswöchig',
  'sechzehnte',
  'sechzigste',
  'sediert',
  'seeklar',
  'seekrank',
  'seelenruhig',
  'seelenvoll',
  'seelisch',
  'seetüchtig',
  'sehenswert',
  'sehnsüchtig',
  'seicht',
  'seiden',
  'seidig',
  'seiger',
  'seimig',
  'seismisch',
  'seismografisch',
  'seismographisch',
  'seitherig',
  'seitlich',
  'sekkant',
  'sektoral',
  'sekündlich',
  'sekundär',
  'sekundenlang',
  'selbständig',
  'selbstbestimmt',
  'selbstbewusst',
  'selbstgemacht',
  'selbstgerecht',
  'selbstherrlich',
  'selbstironisch',
  'selbstkritisch',
  'selbstlos',
  'selbstständig',
  'selbstsüchtig',
  'selbsttätig',
  'selbstverständlich',
  'selektionistisch',
  'selektiv',
  'selig',
  'selten',
  'seltsam',
  'semantisch',
  'semasiologisch',
  'semesterbegleitend',
  'semiotisch',
  'semitisch',
  'senegalesisch',
  'senil',
  'senkrecht',
  'sensibel',
  'sensitiv',
  'sensuell',
  'sentimental',
  'separat',
  'sephardisch',
  'septiert',
  'septisch',
  'sequentiell',
  'sequenziell',
  'serbisch',
  'serbisch-montenegrinisch',
  'serbokroatisch',
  'seriell',
  'serienmäßig',
  'serifenlos',
  'seriös',
  'servil',
  'sesshaft',
  'sexagesimal',
  'sexbesessen',
  'sexistisch',
  'sexual',
  'sexuell',
  'sexy',
  'seychellisch',
  'sezessionistisch',
  'siamesisch',
  'sibyllinisch',
  'sichelförmig',
  'sicher',
  'sichergestellt',
  'sichtbar',
  'sichtbarlich',
  'sichtlaut',
  'sichtlich',
  'siderisch',
  'siebeneckig',
  'siebenfach',
  'siebenstellig',
  'siebenstündig',
  'siebente',
  'siebenwöchig',
  'siebte',
  'siebzehnte',
  'siebzigste',
  'siegesbewusst',
  'siegesgewiss',
  'siegreich',
  'sierra-leonisch',
  'signifikant',
  'silbenbildend',
  'silbentragend',
  'silberfarben',
  'silberhaltig',
  'silbern',
  'silbisch',
  'siliziumhaltig',
  'simbabwisch',
  'simpel',
  'simultan',
  'sinfonisch',
  'singapurisch',
  'singhalesisch',
  'singulär',
  'sinister',
  'sinistral',
  'sinnbildlich',
  'sinnfällig',
  'sinnfrei',
  'sinngemäß',
  'sinnig',
  'sinnlich',
  'sinnlos',
  'sinnstiftend',
  'sinnverwandt',
  'sinnvoll',
  'sintflutartig',
  'siriusfern',
  'sittenwidrig',
  'sittsam',
  'situativ',
  'sizilianisch',
  'skabrös',
  'skandalös',
  'skandinavisch',
  'skatologisch',
  'skeptisch',
  'skoptisch',
  'skrofulös',
  'skrupellos',
  'skrupulös',
  'skurril',
  'skythisch',
  'slawisch',
  'slowakisch',
  'slowenisch',
  'smaragden',
  'smaragdgrün',
  'smart',
  'sodomitisch',
  'sofortig',
  'soigniert',
  'solar',
  'solarthermisch',
  'solenn',
  'solid',
  'solidarisch',
  'solipsistisch',
  'solitär',
  'solubel',
  'solvent',
  'somalisch',
  'somatisch',
  'sommergrün',
  'sommerlich',
  'somnambul',
  'sonderbar',
  'sonderlich',
  'sonnig',
  'sonor',
  'sonstig',
  'sophistisch',
  'sorbisch',
  'sorgenfrei',
  'sorgenlos',
  'sorgfältig',
  'sorglos',
  'sortenrein',
  'soteriologisch',
  'sowjetisch',
  'sozial',
  'sozialistisch',
  'sozialpolitisch',
  'sozioökologisch',
  'sozioökonomisch',
  'spack',
  'spackig',
  'spärlich',
  'spät',
  'spätlateinisch',
  'spätmittelhochdeutsch',
  'spagirisch',
  'spanisch',
  'spannend',
  'sparsam',
  'spartanisch',
  'SPD-geführt',
  'speckig',
  'speditiv',
  'speicherresident',
  'speiübel',
  'spektakulär',
  'spekulativ',
  'speziell',
  'spezifisch',
  'spiegelblank',
  'spiegelglatt',
  'spielerisch',
  'spielsüchtig',
  'spießig',
  'spillerig',
  'spillrig',
  'spinal',
  'spindeldürr',
  'spinnefeind',
  'spinnert',
  'spinös',
  'spitz',
  'spitzenmäßig',
  'spitzfindig',
  'splitterfasernackt',
  'splitternackt',
  'spöttisch',
  'spongiform',
  'spontan',
  'sporadisch',
  'sportiv',
  'sportlich',
  'spottbillig',
  'spottschlecht',
  'spottsüchtig',
  'sprachgewaltig',
  'sprachgewandt',
  'sprachlich',
  'sprachlos',
  'sprachwissenschaftlich',
  'spritig',
  'spröde',
  'spruchreif',
  'sprunghaft',
  'spürbar',
  'spurlos',
  'sri-lankisch',
  'staatlich',
  'stabil',
  'stachelig',
  'städtisch',
  'stählern',
  'stämmig',
  'ständig',
  'stärkehaltig',
  'stalinistisch',
  'standardmäßig',
  'standardsprachlich',
  'standesbewusst',
  'standhaft',
  'standrechtlich',
  'stark',
  'starr',
  'startbereit',
  'startklar',
  'stationär',
  'statisch',
  'statthaft',
  'stattlich',
  'statutarisch',
  'staubig',
  'steif',
  'steigerbar',
  'steil',
  'steinalt',
  'steinern',
  'steinig',
  'steinreich',
  'steirisch',
  'stellar',
  'stellvertretend',
  'stenök',
  'stenohalin',
  'sterblich',
  'stereotyp',
  'steril',
  'sternhagelvoll',
  'stetig',
  'steuerbar',
  'steuerfrei',
  'steuerlich',
  'steuerpolitisch',
  'stichfest',
  'stichhaltig',
  'stichometrisch',
  'stichprobenartig',
  'stichprobenhaft',
  'stickig',
  'stickstoffhaltig',
  'stier',
  'stilistisch',
  'still',
  'stillos',
  'stillschweigend',
  'stilmäßig',
  'stilsicher',
  'stilvoll',
  'stimmhaft',
  'stimmlos',
  'stochastisch',
  'stockdunkel',
  'stockfinster',
  'stocksteif',
  'stocktaub',
  'störrisch',
  'stofflich',
  'stoisch',
  'stolz',
  'stomachal',
  'stotterig',
  'stottrig',
  'strack',
  'strafbar',
  'strafbewehrt',
  'straff',
  'strafmildernd',
  'strafmindernd',
  'strafrechtlich',
  'strafverdächtig',
  'strafverschärfend',
  'stramm',
  'straßenköterblond',
  'strategisch',
  'strauchartig',
  'streitbar',
  'streitbefangen',
  'streitsüchtig',
  'streng',
  'strenggläubig',
  'stressfrei',
  'stressresistent',
  'streufähig',
  'strikt',
  'stringent',
  'strittig',
  'strohblond',
  'strohdumm',
  'strohen',
  'strukturiert',
  'strunzdumm',
  'struppig',
  'stubenrein',
  'studentisch',
  'stündlich',
  'stürmisch',
  'stufenlos',
  'stumm',
  'stumpf',
  'stumpfsinnig',
  'stundenlang',
  'stupend',
  'stupide',
  'stupsnasig',
  'stur',
  'sturmfrei',
  'sturzbetrunken',
  'stutzig',
  'stylish',
  'subaltern',
  'subaquatisch',
  'subarktisch',
  'subdural',
  'subfebril',
  'subfossil',
  'subgingival',
  'subglazial',
  'subjektiv',
  'subkonszient',
  'subkrustal',
  'subkulturell',
  'subkutan',
  'sublim',
  'sublingual',
  'sublunarisch',
  'submarin',
  'submers',
  'submikroskopisch',
  'submissiv',
  'submukös',
  'subnival',
  'suboptimal',
  'suborbital',
  'subperiostal',
  'subphrenisch',
  'subpolar',
  'subsidiär',
  'substantiell',
  'substanziell',
  'subsumtiv',
  'subterran',
  'subtil',
  'subungual',
  'suburbikarisch',
  'subversiv',
  'sudanesisch',
  'süchtig',
  'südafrikanisch',
  'süddeutsch',
  'südkoreanisch',
  'südländisch',
  'südlich',
  'südöstlich',
  'südostasiatisch',
  'südostdeutsch',
  'südsudanesisch',
  'südwestlich',
  'süffig',
  'süffisant',
  'sündhaft',
  'sündteuer',
  'süß',
  'süßlich',
  'suffizient',
  'suggestiv',
  'suizidal',
  'sukzessiv',
  'sulzig',
  'sumerisch',
  'summarisch',
  'summativ',
  'superb',
  'supergen',
  'superlativ',
  'superprovisorisch',
  'supplementär',
  'suppressiv',
  'supragingival',
  'supranational',
  'suprapubisch',
  'surinamisch',
  'surjektiv',
  'surreal',
  'suspekt',
  'suszeptibel',
  'swasiländisch',
  'symbolisch',
  'symmetrisch',
  'sympathetisch',
  'sympathisch',
  'symphonisch',
  'symptomatisch',
  'synchron',
  'synchronisch',
  'syndetisch',
  'synekdochisch',
  'synergistisch',
  'synkretistisch',
  'synonym',
  'synoptisch',
  'synsemantisch',
  'synsystematisch',
  'syntagmatisch',
  'syntaktisch',
  'syntaxonomisch',
  'synthetisch',
  'syntonisch',
  'syrisch',
  'systematisch',
  'systemisch',
  'systemlos',
  'systemoid',
  'szenisch',
  'szientifisch',
  'tabellarisch',
  'tablettensüchtig',
  'tadellos',
  'tadschikisch',
  'täglich',
  'tätig',
  'tätlich',
  'taff',
  'taften',
  'tagelang',
  'tageweise',
  'tagtäglich',
  'taktil',
  'taktisch',
  'taktlos',
  'talentfrei',
  'talentiert',
  'talmudisch',
  'tangibel',
  'tannen',
  'tansanisch',
  'tanzbar',
  'tapfer',
  'tarentinisch',
  'tariflich',
  'tatarisch',
  'tatenlos',
  'tatsächlich',
  'tatterig',
  'taub',
  'taubenblau',
  'taubstumm',
  'tauglich',
  'tausendfach',
  'tausendste',
  'tautologisch',
  'taxativ',
  'teaken',
  'teamfähig',
  'technisch',
  'teilbar',
  'teilerfremd',
  'teilhaftig',
  'tektonisch',
  'telefonisch',
  'telegen',
  'teleoklin',
  'teleologisch',
  'tellurisch',
  'temperamentvoll',
  'temporär',
  'temporal',
  'tendenziell',
  'tendenziös',
  'tentativ',
  'teratogen',
  'terminal',
  'termingerecht',
  'ternär',
  'terrestrisch',
  'terrisch',
  'tertiär',
  'tetragonal',
  'teuer',
  'teuflisch',
  'teutonisch',
  'textuell',
  'thailändisch',
  'thalassogen',
  'theatralisch',
  'thematisch',
  'theodosianisch',
  'theologisch',
  'theoretisch',
  'therapierbar',
  'thermophil',
  'thetisch',
  'thorakal',
  'thüringisch',
  'tief',
  'tiefbewegt',
  'tiefgründig',
  'tiefschürfend',
  'tiefsinnig',
  'tierisch',
  'timokratisch',
  'timoresisch',
  'tirolisch',
  'tobsüchtig',
  'todblass',
  'todernst',
  'todgeweiht',
  'todkrank',
  'todmüde',
  'todschick',
  'tödlich',
  'töfte',
  'tönern',
  'töricht',
  'togoisch',
  'togolesisch',
  'tolerant',
  'toll',
  'tolldreist',
  'tollkühn',
  'tollpatschig',
  'tomatiert',
  'tongaisch',
  'tonhaltig',
  'tonisch',
  'tonnenschwer',
  'tonotop',
  'topasen',
  'topisch',
  'topografisch',
  'toreutisch',
  'torlos',
  'tot',
  'total',
  'totalitär',
  'totenblass',
  'totipotent',
  'traditionalistisch',
  'traditionell',
  'traditionsbewusst',
  'traditionsreich',
  'trächtig',
  'träf',
  'träge',
  'trägerlos',
  'tragbar',
  'tragisch',
  'transdisziplinär',
  'transfinit',
  'transient',
  'transitiv',
  'transitorisch',
  'transkontinental',
  'transmembranös',
  'transparent',
  'transphobisch',
  'transregional',
  'transspezifisch',
  'transversal',
  'transzendent',
  'transzendental',
  'trashig',
  'traulich',
  'traumlos',
  'traurig',
  'traut',
  'trefflich',
  'treife',
  'trendig',
  'trendy',
  'trennbar',
  'treu',
  'treudoof',
  'treusorgend',
  'tribal',
  'trichinenhaltig',
  'triftig',
  'trilliardste',
  'trillionste',
  'trinär',
  'trist',
  'trivial',
  'trocken',
  'trojanisch',
  'tropisch',
  'tropologisch',
  'trotzig',
  'trüb',
  'trunksüchtig',
  'trutzig',
  'tschadisch',
  'tschechisch',
  'tschechoslowakisch',
  'tschetschenisch',
  'tuchen',
  'tüchtig',
  'türkis',
  'türkisch',
  'tugendhaft',
  'tumb',
  'tumultartig',
  'tumultuarisch',
  'tunesisch',
  'turbulent',
  'turkmenisch',
  'turnusmäßig',
  'tuvaluisch',
  'tuwinisch',
  'typisch',
  'tyrrhenisch',
  'übel',
  'überaltert',
  'überbordend',
  'überdacht',
  'überdrüssig',
  'überdurchschnittlich',
  'überfällig',
  'überflüssig',
  'übergewichtig',
  'überglücklich',
  'übergriffig',
  'übergroß',
  'überhastet',
  'überheblich',
  'überirdisch',
  'überkandidelt',
  'überlaufen',
  'überlegen',
  'übermäßig',
  'überraschend',
  'überrascht',
  'überregional',
  'überschaubar',
  'überschwänglich',
  'übersetzt',
  'übersichtlich',
  'übertragbar',
  'übertragen',
  'überwältigend',
  'überwechtet',
  'überwiegend',
  'überzogen',
  'ubiquitär',
  'üblich',
  'übrig',
  'üppig',
  'ugandisch',
  'ukrainisch',
  'ulkig',
  'ultimativ',
  'ultrakonservativ',
  'ultramontan',
  'ultraviolett',
  'umbilikal',
  'umfangreich',
  'umfassend',
  'umgänglich',
  'umgangssprachlich',
  'umgehend',
  'umgerechnet',
  'ummauert',
  'umnachtet',
  'umseitig',
  'umsichtig',
  'umständlich',
  'umstandslos',
  'umstritten',
  'umstürzlerisch',
  'umtriebig',
  'umweltbewusst',
  'umweltfreundlich',
  'umweltpolitisch',
  'umweltschädlich',
  'unabänderlich',
  'unabdingbar',
  'unabdinglich',
  'unabhängig',
  'unabkömmlich',
  'unangemessen',
  'unangenehm',
  'unangesehen',
  'unanständig',
  'unanstößig',
  'unantastbar',
  'unappetitlich',
  'unauffällig',
  'unaufgefordert',
  'unaufhörlich',
  'unausstehlich',
  'unbändig',
  'unbar',
  'unbeabsichtigt',
  'unbedankt',
  'unbedarft',
  'unbedenklich',
  'unbedeutend',
  'unbedingt',
  'unbeeinflussbar',
  'unbeeinflusst',
  'unbefangen',
  'unbefristet',
  'unbefugt',
  'unbegreiflich',
  'unbegründet',
  'unbehaart',
  'unbehaglich',
  'unbeirrbar',
  'unbeirrt',
  'unbekannt',
  'unbekleidet',
  'unbekümmert',
  'unbelebt',
  'unbemannt',
  'unbequem',
  'unberechenbar',
  'unbeschwert',
  'unbesehen',
  'unbesiegbar',
  'unbesiegt',
  'unbespielbar',
  'unbeständig',
  'unbestimmt',
  'unbeteiligt',
  'unbetont',
  'unbeugbar',
  'unbewehrt',
  'unbeweisbar',
  'unbewohnbar',
  'unbewohnt',
  'unbewusst',
  'unbezahlbar',
  'unbezwingbar',
  'unbotmäßig',
  'unbrennbar',
  'unbunt',
  'unchristlich',
  'uncool',
  'undankbar',
  'undeklinierbar',
  'undeutlich',
  'undicht',
  'undulös',
  'undurchdringlich',
  'undurchsichtig',
  'uneben',
  'uneigentlich',
  'uneinnehmbar',
  'unendlich',
  'unentgeltlich',
  'unentschuldbar',
  'unentwegt',
  'unerbittlich',
  'unerfahren',
  'unerforscht',
  'unerfüllbar',
  'unerheblich',
  'unerhört',
  'unerlässlich',
  'unermesslich',
  'unermüdlich',
  'unerquicklich',
  'unerschöpflich',
  'unerschrocken',
  'unerschütterlich',
  'unersprießlich',
  'unerträglich',
  'unerwartet',
  'unerwünscht',
  'unethisch',
  'unfähig',
  'unfassbar',
  'unfasslich',
  'unfehlbar',
  'unflätig',
  'unfreiwillig',
  'unfreundlich',
  'unfruchtbar',
  'ungalant',
  'ungarisch',
  'ungebändigt',
  'ungebildet',
  'ungebräuchlich',
  'ungeduldig',
  'ungefähr',
  'ungefährlich',
  'ungehalten',
  'ungehemmt',
  'ungeheuer',
  'ungeheuerlich',
  'ungekämmt',
  'ungelegen',
  'ungemein',
  'ungenau',
  'ungenießbar',
  'ungenügend',
  'ungenügsam',
  'ungepflegt',
  'ungerade',
  'ungerechnet',
  'ungerecht',
  'ungerundet',
  'ungesäuert',
  'ungeschickt',
  'ungeschlacht',
  'ungeschminkt',
  'ungeschoren',
  'ungeschützt',
  'ungesetzlich',
  'ungespannt',
  'ungeständig',
  'ungestraft',
  'ungestüm',
  'ungesühnt',
  'ungesund',
  'ungewiss',
  'ungewöhnlich',
  'ungezogen',
  'ungiftig',
  'ungläubig',
  'unglaublich',
  'unglaubwürdig',
  'ungleich',
  'unglücklich',
  'unglückselig',
  'ungnädig',
  'ungrammatisch',
  'ungültig',
  'ungut',
  'unhaltbar',
  'unheilbar',
  'unheilvoll',
  'unheimlich',
  'unhistorisch',
  'unhöflich',
  'unhörbar',
  'uniert',
  'uniform',
  'unikal',
  'unikursal',
  'unilateral',
  'uninteressant',
  'unislamisch',
  'unitär',
  'unitaristisch',
  'universal',
  'universell',
  'universitär',
  'unkalkulierbar',
  'unkaputtbar',
  'unklar',
  'unkompliziert',
  'unkonventionell',
  'unkreativ',
  'unkündbar',
  'unkundig',
  'unlauter',
  'unleidig',
  'unlimitiert',
  'unliniert',
  'unliniiert',
  'unlustig',
  'unmenschlich',
  'unmissverständlich',
  'unmittelbar',
  'unmöglich',
  'unmotiviert',
  'unmündig',
  'unnachgiebig',
  'unnahbar',
  'unnötig',
  'unordentlich',
  'unpässlich',
  'unparteiisch',
  'unparteilich',
  'unpassend',
  'unpassierbar',
  'unpathetisch',
  'unpersönlich',
  'unpfändbar',
  'unplatziert',
  'unpopulär',
  'unprätentiös',
  'unrasiert',
  'unrealistisch',
  'unrechtmäßig',
  'unregelmäßig',
  'unreif',
  'unrichtig',
  'unruhig',
  'unsachlich',
  'unsäglich',
  'unsagbar',
  'unschierig',
  'unschön',
  'unschwer',
  'unschwierig',
  'unsentimental',
  'unsexy',
  'unsichtbar',
  'unsichtig',
  'unsinkbar',
  'unsinnig',
  'unsozial',
  'unspielbar',
  'unsportlich',
  'unsterblich',
  'unstet',
  'unsympathisch',
  'unsystematisch',
  'untadelig',
  'untätig',
  'untalentiert',
  'untauglich',
  'untechnisch',
  'unterbewusst',
  'unterdurchschnittlich',
  'untergärig',
  'untergeben',
  'unterhaltsam',
  'unterirdisch',
  'unterjährig',
  'untermeerisch',
  'unterminiert',
  'unterschiedlich',
  'unterschlächtig',
  'unterschwellig',
  'unterseeisch',
  'untersetzt',
  'untertänig',
  'unterwasserarchäologisch',
  'untrennbar',
  'untreu',
  'untüchtig',
  'unüberbrückbar',
  'unüberlegt',
  'unüberschaubar',
  'unübersehbar',
  'unübersetzbar',
  'unübertrefflich',
  'unüblich',
  'unumgänglich',
  'unumgehbar',
  'unumkehrbar',
  'unumstößlich',
  'ununterbrochen',
  'unveränderbar',
  'unverändert',
  'unverbindlich',
  'unverblümt',
  'unverbrüchlich',
  'unverdächtig',
  'unverdrossen',
  'unverformbar',
  'unverfroren',
  'unvergesslich',
  'unverheiratet',
  'unverhofft',
  'unverletzlich',
  'unvermittelt',
  'unvernünftig',
  'unverschämt',
  'unverständlich',
  'unversucht',
  'unverwandt',
  'unverwundbar',
  'unverzagt',
  'unverzichtbar',
  'unverzüglich',
  'unvollendet',
  'unvollständig',
  'unvorschriftsmäßig',
  'unvorstellbar',
  'unwahr',
  'unwahrscheinlich',
  'unwegsam',
  'unweigerlich',
  'unwichtig',
  'unwiderlegbar',
  'unwiderstehlich',
  'unwiederbringlich',
  'unwillkürlich',
  'unwirksam',
  'unwirsch',
  'unwirtlich',
  'unwissenschaftlich',
  'unzählbar',
  'unzeitgemäß',
  'unzerkaut',
  'unzertrennlich',
  'unzial',
  'unzivilisiert',
  'unzüchtig',
  'unzufrieden',
  'unzugänglich',
  'unzulänglich',
  'unzumutbar',
  'unzureichend',
  'unzustellbar',
  'unzutreffend',
  'unzweckmäßig',
  'uralt',
  'uranhaltig',
  'urban',
  'urbar',
  'urcool',
  'urgemütlich',
  'urgermanisch',
  'urig',
  'urkundlich',
  'uroborisch',
  'urologisch',
  'urplötzlich',
  'urslawisch',
  'ursprünglich',
  'uruguayisch',
  'US-amerikanisch',
  'usbekisch',
  'usuell',
  'utilitaristisch',
  'utopisch',
  'uvular',
  'vag',
  'vagil',
  'vaginal',
  'vakant',
  'valabel',
  'valid',
  'vanille',
  'variabel',
  'varikös',
  'vaskulär',
  'vaskularisiert',
  'vaterlos',
  'vatikanisch',
  'vegan',
  'vegetativ',
  'vehement',
  'veilchenblau',
  'veilchenfarben',
  'veilchenfarbig',
  'velar',
  'venenös',
  'venerisch',
  'venetisch',
  'venezianisch',
  'venezolanisch',
  'venös',
  'ventral',
  'verabscheuungswürdig',
  'verächtlich',
  'veränderbar',
  'veraltend',
  'veraltet',
  'verantwortlich',
  'verantwortungsbewusst',
  'verantwortungslos',
  'verbal',
  'verbesserungswürdig',
  'verbeult',
  'verbindlich',
  'verboten',
  'verbreitet',
  'verbunden',
  'verdächtig',
  'verdammt',
  'verderbt',
  'verdrießlich',
  'verdruckst',
  'verdutzt',
  'vereinbarungsgemäß',
  'vereinslos',
  'vereitert',
  'verfänglich',
  'verfahren',
  'verfassungsfeindlich',
  'verflucht',
  'verformbar',
  'verfressen',
  'verfristet',
  'verfügbar',
  'vergänglich',
  'vergammelt',
  'vergeblich',
  'vergesslich',
  'vergleichbar',
  'vergnügungssüchtig',
  'verhältnismäßig',
  'verhärmt',
  'verhalten',
  'verhaltensgestört',
  'verhandlungsfähig',
  'verhascht',
  'verhasst',
  'verheerend',
  'verheiratet',
  'verhohlen',
  'verifiziert',
  'veritabel',
  'verkappt',
  'verkaufsoffen',
  'verkehrt',
  'verklausuliert',
  'verknöchert',
  'verkopft',
  'verkrebst',
  'verlässlich',
  'verlassen',
  'verlegen',
  'verletzungsbedingt',
  'verliebt',
  'verlogen',
  'verlustbehaftet',
  'vermaledeit',
  'vermehrt',
  'vermeintlich',
  'vermessen',
  'vermieft',
  'vermittelbar',
  'vermögend',
  'vermottet',
  'vernakulär',
  'vernehmungsfähig',
  'vernünftig',
  'vernunftbegabt',
  'veronesisch',
  'verpflichtungslos',
  'verpönt',
  'verquast',
  'verrechenbar',
  'verrucht',
  'verrückt',
  'verrunzelt',
  'versandkostenfrei',
  'versaut',
  'verschieden',
  'verschiedenartig',
  'verschlafen',
  'verschlagen',
  'verschlossen',
  'verschmitzt',
  'verschmust',
  'verschneit',
  'verschollen',
  'verschreibungspflichtig',
  'verschroben',
  'verschuldet',
  'verschwenderisch',
  'verschwendungssüchtig',
  'verschwiegen',
  'versehentlich',
  'versicherungspflichtig',
  'versiert',
  'versonnen',
  'verspätet',
  'verständig',
  'verständlich',
  'verständnislos',
  'verständnisvoll',
  'versteinert',
  'verstellbar',
  'verstört',
  'verstorben',
  'vertiert',
  'vertikal',
  'vertrackt',
  'vertränt',
  'vertrauensselig',
  'vertrauensvoll',
  'vertrauenswürdig',
  'vertraulich',
  'vertraut',
  'vertretbar',
  'verwandt',
  'verwegen',
  'verwendbar',
  'verwerflich',
  'verwertbar',
  'verwirrt',
  'verwitwet',
  'verwöhnt',
  'verworren',
  'verwundbar',
  'verwunschen',
  'verzopft',
  'verzückt',
  'verzwickt',
  'vestibulär',
  'vestimentär',
  'vibrierend',
  'videoüberwacht',
  'vielfach',
  'vielfältig',
  'vielgestaltig',
  'vielseitig',
  'vielzellig',
  'vierarmig',
  'viereckig',
  'vierfach',
  'viermotorig',
  'vierschrötig',
  'vierstellig',
  'vierstündig',
  'vierte',
  'vierteljährlich',
  'viertelstündig',
  'viertelstündlich',
  'viertürig',
  'vierwöchig',
  'vierzehnte',
  'vierzigste',
  'vietnamesisch',
  'vigesimal',
  'vigil',
  'viktorianisch',
  'vincentisch',
  'vinophil',
  'violett',
  'viral',
  'viril',
  'virtuell',
  'virtuos',
  'virulent',
  'visafrei',
  'visionär',
  'viskos',
  'visuell',
  'viszeral',
  'vital',
  'vitalistisch',
  'vivipar',
  'völkerrechtswidrig',
  'völkisch',
  'völlig',
  'vogelfrei',
  'vogtländisch',
  'volatil',
  'volitional',
  'volkseigen',
  'volksetymologisch',
  'voll',
  'vollendet',
  'volljährig',
  'vollkommen',
  'vollmundig',
  'vollschlank',
  'vollständig',
  'vollumfänglich',
  'voluntaristisch',
  'voraussichtlich',
  'vorbehältlich',
  'vorbehaltlich',
  'vorbehaltlos',
  'vorbestraft',
  'vorbeugend',
  'vorbildhaft',
  'vorbildlich',
  'vorchristlich',
  'vorderer',
  'vordergründig',
  'voreingenommen',
  'vorfristig',
  'vorgabenwirksam',
  'vorgängig',
  'vorgefasst',
  'vorgerückt',
  'vorhanden',
  'vorherig',
  'vorhersehbar',
  'vorig',
  'vorkonziliar',
  'vorläufig',
  'vorletzte',
  'vorliegend',
  'vormalig',
  'vornehm',
  'vorrätig',
  'vorsätzlich',
  'vorschriftsgemäß',
  'vorschriftsmäßig',
  'vorschriftswidrig',
  'vorsichtig',
  'vorsintflutlich',
  'vorsorglich',
  'vorstellbar',
  'vorteilhaft',
  'vorübergehend',
  'vorweihnachtlich',
  'vorzeigbar',
  'vorzeitig',
  'vorzüglich',
  'vulgär',
  'vulgärlateinisch',
  'vulkanisch',
  'vulkanistisch',
  'vulkanologisch',
  'vulnerabel',
  'waagerecht',
  'waagrecht',
  'wach',
  'wachsam',
  'wacker',
  'wählerisch',
  'wärmeliebend',
  'wässrig',
  'wagnerianisch',
  'wahhabitisch',
  'wahlberechtigt',
  'wahnsinnig',
  'wahr',
  'wahrhaft',
  'wahrhaftig',
  'wahrheitsgemäß',
  'wahrlich',
  'wahrscheinlich',
  'waidmännisch',
  'waldensisch',
  'walisisch',
  'wandlungsfähig',
  'wankelmütig',
  'warm',
  'wasserarm',
  'wasserdicht',
  'wasserdurchlässig',
  'wasserfest',
  'wassergekühlt',
  'wasserlöslich',
  'wasserscheu',
  'wasserundurchlässig',
  'wechselhaft',
  'wechselseitig',
  'wegsam',
  'wehleidig',
  'wehrfähig',
  'wehrig',
  'wehrlos',
  'weibisch',
  'weiblich',
  'weich',
  'weidlich',
  'weidwund',
  'weihnachtlich',
  'weinerlich',
  'weinfarben',
  'weinfarbig',
  'weinfröhlich',
  'weinfroh',
  'weingelb',
  'weinhaltig',
  'weinig',
  'weinrot',
  'weinsauer',
  'weinselig',
  'weise',
  'weiß',
  'weißfeldrig',
  'weißglühend',
  'weißgolden',
  'weißrussisch',
  'weisungsbefugt',
  'weisungsberechtigt',
  'weit',
  'weitentlegen',
  'weitgehend',
  'weitläufig',
  'weitschweifig',
  'welsch',
  'weltanschaulich',
  'weltlich',
  'weltweit',
  'wendisch',
  'werbefrei',
  'werbeträchtig',
  'wergen',
  'wert',
  'wertbeständig',
  'wertkonservativ',
  'wertlos',
  'wertvoll',
  'wesentlich',
  'westdeutsch',
  'westgermanisch',
  'westlich',
  'westmitteldeutsch',
  'westoberdeutsch',
  'westösterreichisch',
  'wettbewerbsfähig',
  'wettbewerbswidrig',
  'wichtig',
  'widerborstig',
  'widerlich',
  'widerrechtlich',
  'widersinnig',
  'widerspenstig',
  'widerspruchslos',
  'widrig',
  'wiedergeboren',
  'wienerisch',
  'wild',
  'wildfremd',
  'wildromantisch',
  'willensstark',
  'willentlich',
  'willfährig',
  'willig',
  'willkommen',
  'willkürlich',
  'windelweich',
  'windig',
  'windschief',
  'winterlich',
  'winzig',
  'wipfelschäftig',
  'wirklich',
  'wirkmächtig',
  'wirksam',
  'wirr',
  'wirsch',
  'wirtschaftlich',
  'wirtschaftspolitisch',
  'wissbegierig',
  'wissenschaftlich',
  'wissenschaftsgeschichtlich',
  'wissenswert',
  'witterungsbeständig',
  'witterungsfest',
  'witzig',
  'wochenlang',
  'wöchentlich',
  'wörtlich',
  'wohlbehalten',
  'wohlbehütet',
  'wohlfeil',
  'wohlgeformt',
  'wohlgenährt',
  'wohlhabend',
  'wohlriechend',
  'wohltätig',
  'wohnlich',
  'wolkenlos',
  'wolkenverhangen',
  'wolkig',
  'wollen',
  'wollüstig',
  'wonnetrunken',
  'wortgetreu',
  'wortgewaltig',
  'wortgewandt',
  'wortkarg',
  'wortlos',
  'wortreich',
  'wortwörtlich',
  'wünschenswert',
  'würdig',
  'würfelförmig',
  'würfelig',
  'würflig',
  'würzig',
  'wüst',
  'wütend',
  'wund',
  'wunderbar',
  'wunderlich',
  'wundersam',
  'wunderschön',
  'wundervoll',
  'wurmstichig',
  'wurzelverwandt',
  'wuschig',
  'wuselig',
  'wutentbrannt',
  'x-beinig',
  'x-beliebig',
  'xenophil',
  'xenophob',
  'xenophontisch',
  'xerographisch',
  'xerophil',
  'x-fach',
  'x-förmig',
  'y-förmig',
  'yttriumhaltig',
  'zäh',
  'zähflüssig',
  'zärtlich',
  'zaghaft',
  'zahlenmäßig',
  'zahlreich',
  'zahlungsfähig',
  'zahlungswirksam',
  'zahm',
  'zahnlos',
  'zapfentragend',
  'zappelig',
  'zappenduster',
  'zapplig',
  'zart',
  'zedern',
  'zehneckig',
  'zehnfach',
  'zehnprozentig',
  'zehnstellig',
  'zehnstündig',
  'zehntausendste',
  'zehnte',
  'zehntel',
  'zehntpflichtig',
  'zehnwöchig',
  'zeitgeistlich',
  'zeitgemäß',
  'zeitgenössisch',
  'zeitgleich',
  'zeitig',
  'zeitinvariant',
  'zeitlich',
  'zeitlos',
  'zeitnah',
  'zeitweilig',
  'zementen',
  'zentral',
  'zentralafrikanisch',
  'zentrifugal',
  'zentripetal',
  'zerebral',
  'zerknirscht',
  'zermürbt',
  'zernarbt',
  'zerstreut',
  'zertalt',
  'zervikogen',
  'zickig',
  'zielbewusst',
  'zielführend',
  'zielgruppengerecht',
  'zimperlich',
  'zinken',
  'zinnen',
  'zinnern',
  'zinnoberrot',
  'zionistisch',
  'zirkadian',
  'zirkulär',
  'zirkumpolar',
  'zisalpin',
  'zitierbar',
  'zitronengelb',
  'zivil',
  'zivilisiert',
  'zivilrechtlich',
  'zögerlich',
  'zollfrei',
  'zombig',
  'zoogeografisch',
  'zoogeographisch',
  'zoologisch',
  'zornentbrannt',
  'zornig',
  'zotig',
  'zottelig',
  'zuckerfrei',
  'zuckerkrank',
  'zuckersüß',
  'züchtig',
  'zügellos',
  'zügig',
  'zünftig',
  'zufällig',
  'zufrieden',
  'zugänglich',
  'zugetan',
  'zugig',
  'zukünftig',
  'zulässig',
  'zungenfertig',
  'zuordenbar',
  'zurechnungsfähig',
  'zurückhaltend',
  'zusätzlich',
  'zusammengehörig',
  'zusammengeritten',
  'zusammenhängend',
  'zuständig',
  'zuträglich',
  'zutraulich',
  'zuverlässig',
  'zuversichtlich',
  'zwanglos',
  'zwangsläufig',
  'zwanzigfach',
  'zwanzigste',
  'zweckdienlich',
  'zweckfrei',
  'zwecklos',
  'zweckmäßig',
  'zweckvoll',
  'zweiarmig',
  'zweibasig',
  'zweibasisch',
  'zweideutig',
  'zweidimensional',
  'zweifach',
  'zweifelhaft',
  'zweifelsfrei',
  'zweihebig',
  'zweihundertste',
  'zweimonatig',
  'zweimotorig',
  'zweisprachig',
  'zweistellig',
  'zweistündig',
  'zweite',
  'zweiteilig',
  'zweitmeist',
  'zweitrangig',
  'zweiundzwanzigste',
  'zweiwöchig',
  'zwiefach',
  'zwielichtig',
  'zwiespältig',
  'zwirnen',
  'zwölfeckig',
  'zwölffach',
  'zwölfstündig',
  'zwölfte',
  'zyanotisch',
  'zygomorph',
  'zyklisch',
  'zylinderförmig',
  'zylindrisch',
  'zynisch',
  'zypressen',
  'zypriotisch',
  'zyprisch',
  'zystisch',
  'zytogen',
  'zytologisch',
  'zytostatisch',
  'zytotoxisch',
  'zytotrop',
  'ängstlich',
  'ätzend',
  'ökologisch',
  'ökonomisch',
  'überdreht',
  'überemotional',
  'überfürsorglich',
  'übergenau',
  'überheblich',
  'überkandidelt',
  'überkritisch',
  'überlebensfähig',
  'überlegen',
  'überlegt',
  'übermütig',
  'überragend',
  'überraschend',
  'übersensibel',
  'überspannt',
  'überwältigend',
  'überzeugend'
];

export const adjectiveEndings = new Map([
  ['f', 'e'],
  ['n', 'es'],
  ['m', 'er']
]);

const returnSelf = value => value;

export const irregularAdjectives = new Map([
  ['apricot', returnSelf],
  ['bleu', returnSelf],
  ['fit', g => `fitt${adjectiveEndings.get(g)}`],
  ['hoch', g => `hoh${adjectiveEndings.get(g)}`],
  ['hip', g => `hipp${adjectiveEndings.get(g)}`],
  ['illuster', g => `illustr${adjectiveEndings.get(g)}`],
  ['lila', returnSelf],
  ['rosé', returnSelf],
  ['rosa', returnSelf],
  ['sexy', returnSelf],
  ['trendy', returnSelf],
  ['unsexy', returnSelf],
  ['vanille', returnSelf]
]);
